...
const AIActionModal = ({ isOpen, onClose, noteContent, onApply, isDark, appTheme, apiKey }: any) => {
    ...
    const handleAction = async (action: string) => {
        ...
        switch (action) {
            case 'summarize': prompt = `Ringkaslah teks ini (Indo). Tanpa markdown/bold:\n\n${content}`; break;
            case 'continue': prompt = `Lanjutkan tulisan ini secara natural. Tanpa markdown:\n\n${content}`; break;
            case 'fix': prompt = `Perbaiki tata bahasa teks ini. Tanpa markdown:\n\n${content}`; break;
            case 'translate': prompt = `Terjemahkan teks ini ke Indo/Inggris. Tanpa markdown:\n\n${content}`; break;
            case 'idea': prompt = `Berikan 1 ide topik jurnal pendek. Tanpa markdown.`; break;
        }
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, isDark, showSettings, onCloseSettings, onOpenSettings, appTheme, apiKey }: any) => {
  ...
  const [selectedImgId, setSelectedImgId] = useState<string | null>(null); // Track ID of selected image
      ...
      if (file) {
          ...
          reader.onload = (event) => {
              if (event.target?.result) {
                  ...
                  // Use execCommand for undo/redo support and proper cursor placement
  ...
  const modifySelectedImage = (action: 'crop' | 'delete', value?: string) => {
      ...
      if (action === 'delete') {
          ...
          // CSS-based cropping simulation using object-fit and aspect-ratio
  ...
  return (
    ...
    >
       ...
       {/* IMAGE CONTEXT MENU (POPOVER) */}
       <AnimatePresence>
           {selectedImgId && (
               <motion.div 
                 initial={{ opacity: 0, scale: 0.9, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 10 }}
                 className="absolute z-50 left-1/2 -translate-x-1/2 bottom-24 shadow-2xl rounded-2xl overflow-hidden"
               >
                   <div className={`flex flex-col p-2 gap-1 min-w-[200px] ${isDark ? 'bg-zinc-800 text-white' : 'bg-white text-zinc-800'}`}>
                        <div className="flex gap-1 p-1 border-b border-black/5 pb-2 mb-1">
                            <button onClick={() => modifySelectedImage('crop', '19:6')} className="flex-1 px-3 py-2 text-xs font-bold rounded-lg hover:bg-black/5 bg-black/5">19:6</button>
                            <button onClick={() => modifySelectedImage('crop', '16:9')} className="flex-1 px-3 py-2 text-xs font-bold rounded-lg hover:bg-black/5 bg-black/5">16:9</button>
                            <button onClick={() => modifySelectedImage('crop', '1:1')} className="flex-1 px-3 py-2 text-xs font-bold rounded-lg hover:bg-black/5 bg-black/5">1:1</button>
                        </div>
                        <button onClick={() => modifySelectedImage('delete')} className="flex items-center gap-2 w-full px-4 py-3 text-sm font-bold text-red-500 hover:bg-red-500/10 rounded-xl transition-colors">
