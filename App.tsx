...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, isDark, showSettings, onCloseSettings, onOpenSettings, appTheme, apiKey }: any) => {
  ...
  // FIX: Undo/Redo & Image Insertion Stability
  // We keep track of the last known HTML to avoid overwriting the DOM when local changes trigger updates
  const lastHtml = useRef(note.content);
  ...
  // FIX: Optimized Content Sync to preserve Undo Stack
  useEffect(() => { 
      if (editorRef.current && note.content !== lastHtml.current) {
          // Only update DOM if the prop changed from outside (not from our own edit)
          // This comparison is a bit naive but works for single-user scenarios
