...
// --- Helper Auto Scroll ---
// Digunakan manual di dalam handler onDrag NoteCard
...
// --- Note Card (PERBAIKAN SENSITIVITAS & SEARCH) ---
...
const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove, setHoveredFolderId, searchQuery }: any) => {
  ...
  // Fungsi saat Long Press terjadi (Timer Selesai)
  ...
  const handlePointerDown = (event: React.PointerEvent) => {
     ...
     if (inFolder) return; 
     if (searchQuery) return; // DISABLE DRAG SAAT SEARCH (FIX 3)

     event.stopPropagation();
     
     // Simpan posisi awal untuk deteksi Swipe
     pointerStartPosition.current = { x: event.clientX, y: event.clientY };

     if (safeIsPeeking) {
         // Peek to Drag: Tunggu animasi shrink selesai
         onPeek(false);
         longPressTimer.current = setTimeout(() => {
             startDragSequence(event);
         }, 200);
     } else {
         // Standard Long Press: 350ms (Tahan agak lama)
         longPressTimer.current = setTimeout(() => {
             startDragSequence(event);
         }, 350); 
     }
  ...
  const handlePointerMove = (event: React.PointerEvent) => {
      // FIX SENSITIVITAS: Jika jari bergerak > 10px sebelum timer habis, batalkan drag
  ...
  const handleDrag = (event: any, info: PanInfo) => {
      ...
      const scrollZone = 100; // 100px dari atas/bawah
      ...
      if (y < scrollZone || y > windowHeight - scrollZone) {
          scrollStep();
      }
      ...
      // Menggunakan posisi jari (info.point) yang lebih akurat daripada elemen rect yang sedang bergerak
  ...
  return (
    <motion.div 
       ...
       onPointerMove={handlePointerMove} // Tambahan handler move
