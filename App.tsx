...
// --- Sidebar Fix (No Blur, Saweria Back) ---
...
// --- Note Editor (FIX: Toolbar Bottom + Header Actions) ---
...
export default function App() {
  ...
  }, [activeNoteId, expandedFolderId, showSidebar, isSelectionMode, showSearch]);

  useEffect(() => {
    if (activeNoteId || expandedFolderId || isSelectionMode || showSearch) {
      window.history.pushState(null, '', window.location.href);
    }
  }, [activeNoteId, expandedFolderId, isSelectionMode, showSearch]);
  ...
  const getFilteredData = () => {
      ...
      if (activeTab === 'favorites') {
          ...
          filteredFolders = []; // Folders can't be pinned in this version logic yet, or just show empty
  ...
  const toggleFavoriteSelected = () => {
      ...
      // Check if majority is pinned to decide toggle direction
      const allPinned = ids.every(id => {
          const n = notes.find(x => x.id === id);
          return n ? n.isPinned : false; 
      });
    ...
    return (
        <div className={`sticky top-0 z-50 pt-4 px-4 pb-2 transition-colors duration-300 ${isDark ? 'bg-[#121212]/95' : 'bg-[#fdfdfd]/95'} backdrop-blur-md`}>
            ...
            {!isSelectionMode && !showSearch && (
                 <motion.div style={{ scale: titleScale, y: titleY, originX: 0 }} className="mb-4">
...
// --- Folder Item (SWIPE TO DELETE ENABLED) ---
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-4 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-3">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    isSelected={false} // Simplification: Selection inside folder not fully synced in this quick fix
                                    onDelete={() => onDeleteNote(note.id)} 
                                    isPeeking={peekingNoteId === note.id}
                                    onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)}
                                    // Handle swipe inside folder
...
// --- Note Card (SWIPE TO DELETE ENABLED) ---
