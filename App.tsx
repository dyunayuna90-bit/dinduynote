...
import { 
  ...
  FilePlus, FolderPlus, Sparkles, Wand2, Languages, Eraser, Bot, Loader2, Copy // Added AI Icons
} from 'lucide-react';
...
export type AppThemeColor = 'violet' | 'blue' | 'green' | 'orange' | 'rose' | 'yellow' | 'teal' | 'indigo' | 'lime' | 'fuchsia';
...
export const APP_THEMES: Record<AppThemeColor, { bg: string, text: string, ring: string, hover: string, border: string, lightBg: string }> = {
    violet: { bg: 'bg-violet-600', text: 'text-violet-600', ring: 'ring-violet-600', hover: 'hover:bg-violet-50', border: 'border-violet-200', lightBg: 'bg-violet-100' },
    blue:   { bg: 'bg-blue-600', text: 'text-blue-600', ring: 'ring-blue-600', hover: 'hover:bg-blue-50', border: 'border-blue-200', lightBg: 'bg-blue-100' },
    green:  { bg: 'bg-emerald-600', text: 'text-emerald-600', ring: 'ring-emerald-600', hover: 'hover:bg-emerald-50', border: 'border-emerald-200', lightBg: 'bg-emerald-100' },
    orange: { bg: 'bg-orange-600', text: 'text-orange-600', ring: 'ring-orange-600', hover: 'hover:bg-orange-50', border: 'border-orange-200', lightBg: 'bg-orange-100' },
    rose:   { bg: 'bg-rose-600', text: 'text-rose-600', ring: 'ring-rose-600', hover: 'hover:bg-rose-50', border: 'border-rose-200', lightBg: 'bg-rose-100' },
    yellow: { bg: 'bg-yellow-500', text: 'text-yellow-600', ring: 'ring-yellow-500', hover: 'hover:bg-yellow-50', border: 'border-yellow-200', lightBg: 'bg-yellow-100' },
    teal:   { bg: 'bg-teal-600', text: 'text-teal-600', ring: 'ring-teal-600', hover: 'hover:bg-teal-50', border: 'border-teal-200', lightBg: 'bg-teal-100' },
    indigo: { bg: 'bg-indigo-600', text: 'text-indigo-600', ring: 'ring-indigo-600', hover: 'hover:bg-indigo-50', border: 'border-indigo-200', lightBg: 'bg-indigo-100' },
    lime:   { bg: 'bg-lime-600', text: 'text-lime-600', ring: 'ring-lime-600', hover: 'hover:bg-lime-50', border: 'border-lime-200', lightBg: 'bg-lime-100' },
    fuchsia:{ bg: 'bg-fuchsia-600', text: 'text-fuchsia-600', ring: 'ring-fuchsia-600', hover: 'hover:bg-fuchsia-50', border: 'border-fuchsia-200', lightBg: 'bg-fuchsia-100' },
};
...
const AIActionModal = ({ isOpen, onClose, noteContent, onApply, isDark, appTheme }: any) => {
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={springTransition} onClick={e => e.stopPropagation()} className={`w-full max-w-md p-6 rounded-[2rem] shadow-2xl max-h-[85vh] overflow-y-auto flex flex-col ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}>
                <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center gap-2 text-indigo-500">
                        <Sparkles size={24} className={theme.text}/>
                        <h3 className={`text-xl font-bold ${theme.text}`}>AI Assistant</h3>
                    </div>
...
// --- MAIN SETTINGS MODAL ---
const SettingsModal = ({ isOpen, onClose, isDark, toggleTheme, onExport, onImport, appTheme, setAppTheme }: any) => {
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="space-y-4">
             {/* THEME SELECTOR - MATERIAL YOU ENGINE */}
            <div className={`p-4 rounded-2xl ${isDark ? 'bg-zinc-800' : 'bg-zinc-100'}`}>
                <h4 className="text-xs font-bold opacity-50 mb-3 uppercase tracking-wider">App Theme (Material You)</h4>
...
// --- Note Editor ---
const NoteEditor = ({ note, onUpdate, onClose, onDelete, isDark, showSettings, onCloseSettings, onOpenSettings, appTheme }: any) => {
  ...
  return (
    ...
    >
       ...
       <AIActionModal isOpen={showAI} onClose={() => setShowAI(false)} noteContent={note.content} onApply={handleAIApply} isDark={isDark} appTheme={appTheme} />
...
export default function App() {
  ...
  const [appTheme, setAppTheme] = useLocalStorage<AppThemeColor>('desnote-app-theme', 'violet');
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-[#121212] text-[#f0f0f0]' : 'bg-[#fdfdfd] text-[#1a1c1e]'}`}>
      <SettingsModal isOpen={showSettingsModal} onClose={() => setShowSettingsModal(false)} isDark={isDark} toggleTheme={() => setIsDark(!isDark)} onExport={handleExport} onImport={handleImport} appTheme={appTheme} setAppTheme={setAppTheme} />
            ...
            <AnimatePresence>
                {showSearch && !isSelectionMode && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-4">
                         <div className={`flex items-center px-4 py-3 rounded-xl ${isDark ? 'bg-zinc-800' : 'bg-zinc-100'} ring-1 ${isDark ? 'ring-zinc-700' : 'ring-zinc-200'} focus-within:ring-2 ${theme.ring}`}>
                    ...
                    {tabs.map(tab => {
                        ...
                        return (
                            <motion.button layout key={tab.id} onClick={() => setActiveTab(tab.id)} initial={false}
                                animate={{ backgroundColor: isActive ? (isDark ? 'var(--bg-zinc-800)' : 'var(--bg-zinc-100)') : (isDark ? '#27272a' : '#f4f4f5'), color: isActive ? '#ffffff' : (isDark ? '#a1a1aa' : '#71717a') }}
                                className={`h-10 px-4 rounded-full flex items-center justify-center flex-shrink-0 relative overflow-hidden transition-colors ${isActive ? theme.bg : ''}`}
                            >
                                <div className={`flex items-center justify-center gap-2 relative z-10 ${isActive ? 'text-white' : ''}`}><Icon size={18} />{isActive && <motion.span initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} className="font-semibold whitespace-nowrap text-sm">{tab.label}</motion.span>}</div>
                            </motion.button>
                        )
                    })}
                    </LayoutGroup>
