...
// --- Note Card (FIX FINAL: SWIPE + DRAG + PEEKING + SCROLL) ---
...
const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove, setHoveredFolderId, searchQuery }: any) => {
  ...
  const startDragSequence = (event: React.PointerEvent) => {
      // PENTING: Jika search aktif, matikan fitur Drag sepenuhnya agar scroll & swipe tetap aman
  ...
  const handlePointerDown = (event: React.PointerEvent) => {
     ...
     // Jangan stopPropagation di sini, biarkan event native bekerja untuk swipe default
     ...
     if (safeIsPeeking) {
         // Jika sedang peeking:
         // 1. Tahan 200ms
         // 2. Set peeking false SEKALIGUS mulai drag
         longPressTimer.current = setTimeout(() => {
             onPeek(false); 
             // Kita butuh trick sedikit agar layout re-render dulu, baru start drag
             // Tapi karena kita pakai state setIsDragging(true), itu akan memicu re-render
             // dan dragControls butuh event asli.
             startDragSequence(event);
         }, 200);
     } else {
         // Standard: Tahan 300ms untuk switch dari Swipe ke Drag
         longPressTimer.current = setTimeout(() => {
             startDragSequence(event);
         }, 300); 
     }
  ...
  // --- AUTO SCROLL LOGIC (Global via requestAnimationFrame) ---
  ...
  useEffect(() => {
      ...
      const checkScroll = () => {
          ...
          const scrollZone = 120; // Area sensitif scroll
          const speed = 15; // Kecepatan scroll
  ...
  const handleDragMove = (event: any, info: PanInfo) => {
      ...
      // Collision Detection Manual (Akurat)
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     ...
     // DROP LOGIC (Hanya jika sedang Dragging)
     setIsDragging(false);
  ...
  return (
    <motion.div 
       ...
       // Pasang listener Pointer di Container Luar
    ...
    >
        <motion.div
           ref={cardRef}
           // --- KUNCI FIX SWIPE & DRAG ---
           // Default: drag="x" (Swipe jalan normal)
           // Saat Long Press: isDragging=true -> Switch ke Free Drag
           drag={isDragging ? true : "x"} 
           
           dragControls={dragControls}
           dragListener={true} // Biarkan true agar Swipe jalan
           
           // Constraints berubah dinamis
           dragConstraints={isDragging ? undefined : { left: 0, right: 0 }}
           
           // Matikan elastic saat drag drop agar tidak mental
           dragElastic={isDragging ? 0 : 0.5}
           
           // Selalu kembali ke asal jika dilepas (kecuali logic onMove menghandle pindah)
           ...
           `}
           // PENTING: pan-y mengizinkan scroll vertical saat swipe horizontal
           // Tapi saat dragging, kita prevent default via logic lain atau biarkan (karena snapToCursor)
