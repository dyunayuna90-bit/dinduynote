...
import { 
  ...
  FilePlus, FolderPlus, Sparkles, Wand2, Languages, Eraser, Bot, Loader2, Copy, Eye, EyeOff, Save, Type, Image as ImageIcon, Crop, Move, Maximize
} from 'lucide-react';
    ...
    return (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/50" onClick={onClose}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={springTransition} onClick={e => e.stopPropagation()} className={`w-full max-w-md p-6 rounded-[2rem] shadow-2xl max-h-[85vh] overflow-y-auto flex flex-col ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}>
                ...
                {result && (<div className="flex flex-col flex-1 overflow-hidden"><div className={`p-4 rounded-2xl mb-4 text-sm leading-relaxed overflow-y-auto max-h-[40vh] whitespace-pre-wrap ${isDark ? 'bg-zinc-800' : 'bg-zinc-100'}`}>{result}</div><div className="flex gap-2"><button onClick={() => { onApply(result, 'append'); onClose(); }} className={`flex-1 py-3 rounded-xl font-bold text-white active:scale-95 transition-transform ${theme.bg}`}>+ Tambah</button><button onClick={() => { onApply(result, 'replace'); onClose(); }} className={`flex-1 py-3 rounded-xl font-bold border active:scale-95 transition-transform ${isDark ? 'border-zinc-700 hover:bg-zinc-800' : 'border-zinc-200 hover:bg-zinc-100'}`}>Ganti</button></div></div>)}
            </motion.div>
        </div>
    )
}
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, isDark, showSettings, onCloseSettings, onOpenSettings, appTheme, apiKey }: any) => {
  ...
  const handleAIApply = (text: string, mode: 'replace' | 'append') => {
      ...
      if (mode === 'replace') {
          // PRESERVE IMAGES: Extract all images first
          const tempDiv = document.createElement('div');
          tempDiv.innerHTML = note.content;
          const images = tempDiv.getElementsByTagName('img');
          const savedImagesHTML = Array.from(images).map(img => img.outerHTML).join('<br/>'); // Stack images

          // Combine AI text + Saved Images at top (Smart Placement)
          const newContent = (savedImagesHTML ? savedImagesHTML + '<br/><br/>' : '') + text;
...
const Header = ({ activeTab, setActiveTab, toggleSettings, isSelectionMode, toggleSelectionMode, isDark, searchQuery, setSearchQuery, showSearch, setShowSearch, selectedCount, deleteSelected, onMoveClick, hasFolderSelected, toggleFavoriteSelected, appTheme }: any) => {
    ...
    // CUSTOM HEADER TEXT MAPPING
    const getHeaderTitle = () => {
        if (activeTab === 'all') return "Your Space";
        if (activeTab === 'favorites') return "Keeping These?";
        if (activeTab === 'folders') return "Everything In Place";
        if (activeTab === 'notes') return "Freshly Captured";
        if (activeTab === 'trash') return "Ready to Clear?";
        return "Desnote";
    };
