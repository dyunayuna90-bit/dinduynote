...
import { 
  ...
  Undo, Redo, Pin, PinOff // Added Pin Icons
} from 'lucide-react';
...
// FIX 1: Enhanced Viewport Hook for Toolbar Position
...
// --- Modals ---
// (DeleteConfirmationModal, MoveDialog, NoteSettingsModal, FolderSettingsModal - Kept same, mostly styling)
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={springTransition} onClick={e => e.stopPropagation()} className={`w-full max-w-md p-6 rounded-[2rem] shadow-2xl max-h-[85vh] overflow-y-auto ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}>
                <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold">Folder Settings</h3><button onClick={onClose} className="p-2 rounded-full hover:bg-black/5"><X size={24}/></button></div>
                 ...
                 {/* Fix 5: Pin Toggle in Folder Settings */}
                 <div className="mb-6 p-4 rounded-2xl bg-black/5 flex justify-between items-center">
                   <div className="flex items-center gap-3">
...
// --- Note Editor (FIX 1: KEYBOARD & GAP) ---
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark, showSettings, onCloseSettings, onOpenSettings }: any) => {
  ...
  // FIX 1: Use visualViewport properties
  ...
  return (
    // FIX 1: Style wrapper with fixed positioning logic
    <div 
    ...
    >
       ...
       {/* Toolbar - FIXED at bottom to avoid gaps */}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // FIX 3: Prevent pull-to-refresh interfering with drag
...
const Dashboard = ({ 
  ...
  // FIX 5: SORTING LOGIC (Pinned First) & FIX 4: Grid Logic
  ...
  return (
    <div className="pb-32 px-4 pt-6 md:px-8 max-w-7xl mx-auto min-h-screen" onClick={() => { setExpandedFolderId(null); if (isSelectionMode) handleExitSelection(); }}>
      ...
      <LayoutGroup>
        {/* FIX 4: Grid System (2 columns mobile, more on desktop) */}
...
const FolderItem = ({ 
  ...
  peekingNoteId, setPeekingNoteId, isHovered, setHoveredFolderId
}: any) => {
  ...
  return (
    ...
    >
        {/* FIX 2: Removed 'overflow-hidden' from main wrapper so dragged items can exit */}
...
// --- Note Card (FIX 3: LOCK SCROLL & FIX 2: DRAG FROM FOLDER) ---

const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove, setHoveredFolderId, searchQuery, onTogglePin }: any) => {
  ...
  // FIX 3: Detect Drag Start to Lock Scroll
  useEffect(() => {
    if (isDragging) {
  ...
  const handlePointerDown = (event: React.PointerEvent) => {
     ...
     // Detect Long Press for Dragging
     longPressTimer.current = setTimeout(() => {
         if (navigator.vibrate) navigator.vibrate(50);
         setIsDragging(true);
         // Important: trigger framer motion drag manually
  ...
  const handlePointerMove = () => {
      ...
      if (longPressTimer.current && !isDragging) {
         // Optionally add distance check logic here if needed, 
         // but usually pointerUp clears it enough for taps.
      }
  };
  ...
  const handleDragMove = (event: any, info: PanInfo) => {
      ...
      // Get pointer position from info
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     ...
     if (!isDragging) { // Was a swipe, not a long-press drag
         if (Math.abs(info.offset.x) > 100 && !isSelectionMode && !inFolder && !searchQuery) {
             onDelete();
         }
         return;
     }

     // Logic Drop to Folder
     const folders = document.querySelectorAll('[data-folder-id]');
     let foundFolderId: string | null = null;
     const pointerX = info.point.x;
     const pointerY = info.point.y;
     ...
     if (foundFolderId && foundFolderId !== note.folderId) {
         ...
         // Dragged OUT of folder to Dashboard (area outside any folder)
         // Calculate if dropped far enough away? 
         // For now, simpler: user must drop on "Dashboard" header or specific area? 
         // Or easier: if dropped outside CURRENT folder bounds.
         // Let's assume dropping on "nothing" in Dashboard view = move to root?
         // This is tricky. Let's keep it simple: Drag to other folder works.
         // To move to root, maybe we need a drop target.
         // Current logic: If dragging FROM folder and dropped on nothing, maybe remove from folder?
         // Let's stick to explicit moves for now or MoveDialog.
         // But user requested Drag from Folder.
         if (Math.abs(info.offset.y) > 200) { // If dragged far away vertically
  ...
  return (
    <motion.div 
       ...
       transition={springTransition}
       // FIX 2: Z-Index max when dragging so it floats above everything
    ...
    >
        <motion.div
           ref={cardRef}
           // FIX 3: Conditional Drag. 
           // If Dragging (via Long Press), allow FREE drag (both axes).
           // If NOT dragging, allow X-axis swipe ONLY if not in folder.
