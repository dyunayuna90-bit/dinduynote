...
const AIActionModal = ({ isOpen, onClose, noteContent, onApply, isDark, appTheme, apiKey }: any) => {
    ...
    const handleAction = async (action: string) => {
        ...
        switch (action) {
            case 'summarize': prompt = `Ringkaslah teks ini (Indo). Langsung poin inti. Tanpa pengantar, tanpa markdown:\n\n${content}`; break;
            case 'continue': prompt = `Lanjutkan tulisan ini. Langsung sambung. Tanpa pengantar, tanpa markdown:\n\n${content}`; break;
            case 'fix': prompt = `Perbaiki tata bahasa teks ini. Langsung hasil perbaikan. Tanpa pengantar, tanpa markdown:\n\n${content}`; break;
            case 'translate': prompt = `Terjemahkan teks ini ke (Inggris/Indonesia, balikannya). Langsung hasil terjemahan. JANGAN ULANG TEKS ASLI. Tanpa pengantar, tanpa markdown:\n\n${content}`; break;
            case 'idea': prompt = `1 ide topik jurnal pendek. Langsung ide. Tanpa pengantar, tanpa markdown.`; break;
        }
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, isDark, showSettings, onCloseSettings, onOpenSettings, appTheme, apiKey }: any) => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
          // CHECK FILE SIZE (5MB Limit to prevent crash)
