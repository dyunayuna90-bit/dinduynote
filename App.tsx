...
// FIX: Ensure correct import path or define constants directly if file is missing/named differently
// Assuming constants.tsx is available. If not, I will inline them to be safe.
...
export default function App() {
  ...
  const requestDelete = (type: string, id: string) => {
      // Logic: If in trash, ask for permanent delete confirmation. If not, normal delete.
      if (activeTab === 'trash') {
         setDeleteModal({ 
             isOpen: true, 
             type, 
             id, 
             actionLabel: "Destroy Forever",
             message: "This item will be permanently deleted and cannot be recovered." 
         });
      } else {
         if (suppressDelete) executeDelete(type, id);
         else setDeleteModal({ isOpen: true, type, id, actionLabel: "Delete", message: "This item will be moved to trash." });
      }
  };
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `desnote-backup-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-[#121212] text-[#f0f0f0]' : 'bg-[#fdfdfd] text-[#1a1c1e]'}`}>
      ...
      <DeleteConfirmationModal isOpen={deleteModal.isOpen} onClose={() => setDeleteModal({...deleteModal, isOpen: false})} isDark={isDark} message={deleteModal.message} actionLabel={deleteModal.actionLabel}
...
const Dashboard = ({ 
  ...
  const handleLongPress = (id: string) => {
      // FIX 2: ENABLE LONG PRESS EVERYWHERE (INCLUDING TRASH)
      if (!isSelectionMode) {
...
const FolderItem = ({ 
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
      // FIX 2: ALLOW INTERACTION IN TRASH FOR SELECTION
  ...
  const onDragEnd = (event: any, info: PanInfo) => {
      // FIX 2: SWIPE TO DELETE ENABLED FOR TRASH AS WELL (PERMANENT DELETE TRIGGER)
...
const NoteCard = ({ note, onClick, onDelete, onRestore, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, searchQuery, isTrash, onLongPress }: any) => {
  ...
  const handlePointerDown = () => {
      // FIX 2: ALLOW LONG PRESS IN TRASH
