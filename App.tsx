...
// --- Note Editor (FIXED KEYBOARD) ---const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark, showSettings, onCloseSettings, onOpenSettings }: any) => {
  ...
  // FORCING METHOD: VisualViewport Listener
  ...
  return (
    <div 
        ...
        style={{ height: `${viewportHeight}px` }} // FORCE HEIGHT
    >
       ...
       <div className={`flex-1 overflow-y-auto px-6 py-6 bg-transparent`}>
          <div className="max-w-4xl mx-auto pb-4">
              ...
              <div ref={editorRef} contentEditable suppressContentEditableWarning onInput={(e) => { onUpdate(note.id, {content: e.currentTarget.innerHTML}); checkFormats(); }} onKeyUp={checkFormats} onMouseUp={checkFormats} onTouchEnd={checkFormats} className="outline-none text-xl leading-relaxed min-h-[40vh] empty:before:content-['Type_something...'] empty:before:opacity-50 text-left [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5" dir="ltr" />
          </div>
       </div>
       <div className={`flex-none w-full p-2 pb-safe ${isDark ? 'bg-zinc-900' : 'bg-white'}`}>
...
export default function App() {
  ...
  const executeDelete = (type: string, id: string | null) => {
      ...
      if (type === 'note') { 
          ...
          // VOMIT LOGIC: Pindahkan isi folder ke root & Paling ATAS
          const notesToVomit = notes.filter(n => n.folderId === id);
          const otherNotes = notes.filter(n => n.folderId !== id);
          
          const updatedVomitedNotes = notesToVomit.map(n => ({
              ...n,
              folderId: null,
              updatedAt: Date.now() // Update time biar fresh
          }));

          // Masukkan ke array PALING DEPAN
          setNotes([...updatedVomitedNotes, ...otherNotes]);
  ...
  // Logic khusus untuk Swipe Note dalam folder: Move to Top of Root
  const handleSwipeNoteInFolder = (noteId: string) => {
      const targetNote = notes.find(n => n.id === noteId);
      const otherNotes = notes.filter(n => n.id !== noteId);
      if (targetNote) {
          const updatedNote = { ...targetNote, folderId: null, updatedAt: Date.now() };
          setNotes([updatedNote, ...otherNotes]); // Pindah ke paling atas
      }
  };
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          ...
          <Dashboard 
            ...
            onDeleteFolder={(id) => requestDelete('folder', id, "Folder ini akan dihapus, dan isinya dimuntahkan ke Dashboard paling atas.")} 
            onDeleteNote={(id) => requestDelete('note', id)}
            onSwipeNoteInFolder={handleSwipeNoteInFolder}
...
const Dashboard = ({ 
  ...
  onMoveClick, hoveredFolderId, setHoveredFolderId, onSwipeNoteInFolder
}: any) => {
             ...
             <>
               {folders.map((folder: Folder) => (
                  <FolderItem 
                    ...
                    onDelete={() => onDeleteFolder(folder.id)} // SWIPE DELETE FOLDER
                    ...
                    onDeleteNote={onSwipeNoteInFolder} // SWIPE NOTE IN FOLDER -> MOVE TO TOP
...
// --- Folder Item (SWIPEABLE & DARKEN ON HOVER) ---
        ...
        <div className={`relative w-full rounded-3xl ${isExpanded ? '' : 'h-full'}`}>
            <motion.div
                ...
                animate={{ 
                    ...
                    // DARKEN EFFECT SAAT DI HOVER NOTE
                    filter: (isHovered && !isExpanded) ? (isDark ? 'brightness(1.3)' : 'brightness(0.85)') : 'brightness(1)'
                }}
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-6 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-4">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    onDelete={() => onDeleteNote(note.id)} // Ini trigger Move to Top
                                    isPeeking={peekingNoteId === note.id}
                                    onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)}
...
// --- Note Card (FIX FINAL: HITBOX BUFFER) ---
...
const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove, setHoveredFolderId, searchQuery }: any) => {
  ...
  const handleDragMove = (event: any, info: PanInfo) => {
      ...
      // HITBOX BUFFER: EXPANDED DETECTION AREA
      const BUFFER = 50; // Pixel buffer (lebih mudah kena)
      ...
      folders.forEach((el) => {
          ...
          // Check if pointer is inside rect + buffer
          if (
