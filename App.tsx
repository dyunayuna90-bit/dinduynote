...
// UPDATED: Faster Animation (Higher Stiffness)
const springTransition = { type: "spring", stiffness: 700, damping: 35, mass: 0.8 };

// UPDATED: Utility to clean HTML and Entities (fixes &nbsp; bug)
...
export default function App() {
  ...
  const handleMoveSelected = (targetId: string | null) => {
      ...
      alert(`Berhasil memindahkan ${itemsToMove.length} catatan!`);
  };
  ...
  // DRAG & DROP: Single Move Function
  const moveNoteToFolder = (noteId: string, folderId: string) => {
      setNotes(prev => prev.map(n => n.id === noteId ? { ...n, folderId: folderId, updatedAt: Date.now() } : n));
  };
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          ...
          <Dashboard 
            ...
            onMoveToFolder={moveNoteToFolder} // Passing move function
...
const Dashboard = ({ 
  ...
  onOpenNote, onUpdateFolder, onDeleteFolder, onDeleteNote, onCreateNoteInFolder, onEjectNote, onMoveToFolder,
             ...
             <>
               ...
               {rootNotes.map((note: Note) => (
                  <NoteCard 
                    ...
                    onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)}
                    onMoveToFolder={onMoveToFolder} // Pass to Root Notes
                  />
               ))}
             </>
           )}
...
// --- Note Card (Updated: Drag Drop Logic & Preview Fix) ---

const NoteCard = ({ note, onClick, onDelete, onMoveToFolder, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek }: any) => {
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     // 1. Swipe to Delete (X axis dominant)
     if (Math.abs(info.offset.x) > 100 && Math.abs(info.offset.y) < 100 && !isSelectionMode) {
        if (safeIsPeeking && onPeek) onPeek(false); // Close peek before delete
        onDelete();
        return;
     }

     // 2. Drag to Folder (Only if not in folder already and not peeking)
     if (!inFolder && !safeIsPeeking && !isSelectionMode && onMoveToFolder) {
         // Get elements under the pointer (Drop location)
         const elements = document.elementsFromPoint(info.point.x, info.point.y);
         const folderElement = elements.find((el) => el.hasAttribute('data-folder-id'));
         
         if (folderElement) {
             const targetFolderId = folderElement.getAttribute('data-folder-id');
             if (targetFolderId) {
                 onMoveToFolder(note.id, targetFolderId);
             }
         }
     }
  };

  // Fix &nbsp; bug by stripping HTML entities
  const previewText = stripHtml(note.content).trim() || "No content";
  ...
  return (
    ...
    >
        <div className="relative w-full h-full">
            <motion.div
               // UPDATED: Allow Drag (Free if root, X only if folder/peek) 
               // If in Dashboard (root) -> Free drag (true) to allow dropping on folder.
               // If in Folder or Peeking -> Constrained X for delete/dismiss.
               drag={isSelectionMode ? false : (!inFolder && !safeIsPeeking ? true : "x")} 
               dragConstraints={{ left: 0, right: 0, top: 0, bottom: 0 }} // Snap back to origin
               dragElastic={{ left: 0.5, right: 0.5, top: 0.5, bottom: 0.5 }} 
               dragSnapToOrigin={true} // Always snap back unless handled
               onDragEnd={handleDragEnd}
               whileDrag={{ scale: 1.1, zIndex: 100, rotate: 2 }} // Visual feedback while dragging
