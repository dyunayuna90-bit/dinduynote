...
// --- CONSTANTS ---
// SATSET ANIMATION TUNING: Higher stiffness = faster, Lower damping = less resistance
...
// --- NEW COMPONENT: DUAL MODE QUICK CARD (GEPENG STYLE) ---
const QuickCreateCard = ({ onCreateNote, onCreateFolder, onFullEditor, appTheme, isDark }: any) => {
    ...
    return (
        <motion.div
            initial={{ height: 85 }} // Compact initial height
            animate={{ height: isOpen ? '55vh' : 85 }}
            transition={menuTransition} // Use faster spring
            className={`fixed bottom-0 left-0 right-0 z-[80] rounded-t-[1.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.15)] border-t border-black/5 flex flex-col overflow-hidden ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-100'}`}
        >
            {/* --- COLLAPSED STATE (THE GEPENG DOCK) --- */}
            <motion.div 
                className="w-full h-[85px] flex flex-col px-4 pt-4 pb-2 flex-none"
                drag="y"
                dragConstraints={{ top: 0, bottom: 0 }}
                dragElastic={0.05} // Stiffer drag
                onDragEnd={(e, info) => {
                    if (info.offset.y < -30) setIsOpen(true); // Sensitive trigger
            ...
            >
                <AnimatePresence mode="wait">
                    {!isOpen ? (
                        ...
                        >
                            ...
                            <div className="flex gap-3 flex-1">
                                <button 
                                    ...
                                    className={`flex-1 rounded-xl font-bold text-sm tracking-wide active:scale-95 transition-transform flex items-center justify-center ${isDark ? 'bg-zinc-800 text-zinc-300 active:bg-zinc-700' : 'bg-gray-100 text-zinc-600 active:bg-gray-200'}`}
                                >
                                    Folder
                                </button>
                                <button 
                                    onClick={(e) => { e.stopPropagation(); onFullEditor(); }}
                                    className={`flex-1 rounded-xl font-bold text-sm tracking-wide active:scale-95 transition-transform flex items-center justify-center ${theme.bg} text-white shadow-sm`}
                                >
                                    Catatan
                                </button>
                            </div>

                            {/* HANDLE BAR (GARIS DOANG) */}
                            <div className="flex justify-center items-center h-4 cursor-grab active:cursor-grabbing" onClick={() => setIsOpen(true)}>
                                <div className={`w-16 h-1 rounded-full opacity-20 ${isDark ? 'bg-white' : 'bg-black'}`} />
                            </div>
                        </motion.div>
                    ) : (
                        // Expanded Header
                        <motion.div 
                            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                            className="flex items-center justify-between w-full h-full pb-2"
                        >
                            <span className="font-bold text-lg opacity-50 ml-2">Quick Note</span>
                            <button onClick={() => setIsOpen(false)} className="p-2 rounded-full hover:bg-black/5 active:scale-90 transition-transform"><ChevronDown size={24}/></button>
                        </motion.div>
                    )}
                </AnimatePresence>
            </motion.div>
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          ...
          <motion.div 
              ...
              onPanEnd={(e, info) => {
                  // FIX BUG: Check if swipe started on an interactive element (Note/Folder)
                  // If closest element has 'interactive-card' class, IGNORE swipe
...
// --- Folder Item ---
const FolderItem = ({ folder, notes, isExpanded, isDark, colors, onToggle, onClose, onOpenNote, onDelete, onUpdate, onCreateNote, onRestore, isSelectionMode, isSelected, onSelect, onDeleteNote, onSettings, peekingNoteId, setPeekingNoteId, isTrash, onLongPress, toggleSelection, selectedIds, appTheme, onNoteLongPress }: any) => {
  ...
  // Added 'interactive-card' class for swipe detection
  return (
