...
// --- Note Editor (FIX: Auto-Scroll & Fixed Toolbar) ---
const NoteEditor = ({ note, onUpdate, onClose, onDelete, isDark, showSettings, onCloseSettings, onOpenSettings }: any) => {
  ...
  const scrollToCursor = () => {
      ...
      // Toolbar height is approx 60px + some padding. Let's say 100px buffer.
      const buffer = 150; 
      
      if (rect.bottom > viewportHeight - buffer) {
          // Scroll the current element into view, or just scroll window
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex flex-col h-[100dvh] ${themeClass}`}>
       ...
       {/* Editor Area */}
       <div className={`flex-1 overflow-y-auto px-6 py-6 bg-transparent pb-32`}>
       ...
       {/* Toolbar - FIXED at bottom with Z-Index 60 to stay above everything */}
       <div className={`fixed bottom-0 left-0 right-0 w-full p-2 ${isDark ? 'bg-zinc-900' : 'bg-white'} shadow-[0_-4px_20px_rgba(0,0,0,0.2)] z-[120]`}>
...
export default function App() {
  ...
  // --- Satpam Back Button Logic (STRICT HIERARCHY) ---
  useEffect(() => {
    // Push state on UI changes
    ...
    const handlePopState = (event: PopStateEvent) => {
      ...
      // 2. Peeking Note (Must close BEFORE Folder)
      if (peekingNoteId) { setPeekingNoteId(null); return; }

      // 3. Editing Note
      if (activeNoteId) { setActiveNoteId(null); return; }
      
      // 4. Expanded Folder (Must close after Peeking is gone)
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-4 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-3">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)}
                                    // FIX: Pass actual toggler to note card inside folder
