...
import { motion, AnimatePresence, LayoutGroup, PanInfo, useDragControls, useMotionValue } from 'framer-motion';
import { 
    ...
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-6 justify-center opacity-70 cursor-pointer" onClick={() => setSuppress(!suppress)}>
           <div className={`w-5 h-5 rounded border-2 border-current flex items-center justify-center ${suppress ? 'bg-indigo-500 border-indigo-500 text-white' : ''}`}>{suppress && <Check size={14}/>}</div>
...
const MoveDialog = ({ isOpen, onClose, onConfirm, folders, isDark, selectedCount }: any) => {
    ...
    return (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={springTransition}
            ...
            >
               <div className="flex justify-between items-center mb-4 flex-none"><h3 className="text-xl font-bold">Pindahkan ({selectedCount})</h3><button onClick={onClose}><X size={24} className="opacity-50"/></button></div>
...
const NoteSettingsModal = ({ isOpen, onClose, note, onUpdate, folders, isDark, colors }: any) => {
    if (!isOpen) return null;
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={springTransition} onClick={e => e.stopPropagation()} className={`w-full max-w-md p-6 rounded-[2rem] shadow-2xl max-h-[85vh] overflow-y-auto ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}>
                ...
                <div className="mb-6 p-4 rounded-2xl bg-black/5 flex justify-between items-center">
                   <div className="flex items-center gap-3"><Pin size={20} className={note.isPinned ? "text-indigo-500" : "opacity-50"} /><span className="font-bold">Pin Note</span></div>
                   <button onClick={() => onUpdate(note.id, { isPinned: !note.isPinned })} className={`w-12 h-7 rounded-full transition-colors relative ${note.isPinned ? 'bg-indigo-500' : 'bg-zinc-300 dark:bg-zinc-700'}`}><div className={`absolute top-1 w-5 h-5 rounded-full bg-white transition-all ${note.isPinned ? 'left-6' : 'left-1'}`} /></button>
                </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={springTransition} onClick={e => e.stopPropagation()} className={`w-full max-w-md p-6 rounded-[2rem] shadow-2xl max-h-[85vh] overflow-y-auto ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}>
                <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold">Folder Settings</h3><button onClick={onClose} className="p-2 rounded-full hover:bg-black/5"><X size={24}/></button></div>
                 <div className="mb-6 p-4 rounded-2xl bg-black/5 flex justify-between items-center">
                   <div className="flex items-center gap-3"><Pin size={20} className={folder.isPinned ? "text-indigo-500" : "opacity-50"} /><span className="font-bold">Pin Folder</span></div>
                   <button onClick={() => onUpdate(folder.id, { isPinned: !folder.isPinned })} className={`w-12 h-7 rounded-full transition-colors relative ${folder.isPinned ? 'bg-indigo-500' : 'bg-zinc-300 dark:bg-zinc-700'}`}><div className={`absolute top-1 w-5 h-5 rounded-full bg-white transition-all ${folder.isPinned ? 'left-6' : 'left-1'}`} /></button>
                </div>
      ...
      {isOpen && (
        <>
          ...
          <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'tween', duration: 0.3, ease: "easeOut" }} className={`fixed top-0 left-0 bottom-0 w-80 z-[70] p-6 shadow-2xl overflow-y-auto ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}>
            <div className="flex justify-between items-center mb-8"><h2 className="text-2xl font-bold">Desnote</h2><button onClick={onClose} className="p-2 rounded-full hover:bg-black/10"><X size={24} /></button></div>
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark, showSettings, onCloseSettings, onOpenSettings }: any) => {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex flex-col ${themeClass} h-[100dvh]`}>
       ...
       <div className={`flex-none flex justify-between items-center p-4 bg-transparent`}>
          ...
          <div className="flex gap-2 relative">
             <IconButton icon={note.isPinned ? PinOff : Pin} onClick={() => onUpdate(note.id, { isPinned: !note.isPinned })} active={note.isPinned} />
...
export default function App() {
  ...
  const [folders, setFolders] = useLocalStorage<Folder[]>('desnote-folders', [
    { id: '1', name: 'Personal', color: 'rose', shape: 'rounded-tl-[2.5rem] rounded-br-[2.5rem] rounded-tr-xl rounded-bl-xl', isPinned: false },
    { id: '2', name: 'Work', color: 'blue', shape: 'rounded-tr-[4rem] rounded-bl-[4rem] rounded-tl-xl rounded-br-xl', isPinned: false }
  ]);
  ...
  const createFolder = () => {
    const newFolder: Folder = { id: uuidv4(), name: 'New Folder', color: 'violet', shape: 'rounded-tl-[2.5rem] rounded-br-[2.5rem] rounded-tr-xl rounded-bl-xl', isPinned: false };
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          ...
          <Dashboard 
            ...
            togglePinNote={(id: string) => setNotes(prev => prev.map(n => n.id === id ? {...n, isPinned: !n.isPinned} : n))}
            isDraggingGlobal={isDraggingGlobal} setIsDraggingGlobal={setIsDraggingGlobal}
          />
        )}
      </AnimatePresence>
...
const Dashboard = ({ 
  ...
  onMoveClick, hoveredFolderId, setHoveredFolderId, togglePinNote,
  isDraggingGlobal, setIsDraggingGlobal
}: any) => {
  const rootNotes = notes.filter((n: Note) => n.folderId === null);
  const colors = isDark ? DARK_COLORS : LIGHT_COLORS;

  const sortedFolders = [...folders].sort((a, b) => (b.isPinned === a.isPinned ? 0 : b.isPinned ? 1 : -1));
  const sortedRootNotes = [...rootNotes].sort((a, b) => {
  ...
  return (
    <div className="pb-32 px-4 pt-6 md:px-8 max-w-7xl mx-auto min-h-screen" onClick={() => { setExpandedFolderId(null); if (isSelectionMode) handleExitSelection(); }}>
      ...
      <LayoutGroup>
        <motion.div layout transition={springTransition} className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 auto-rows-min">
           {searchQuery ? filteredNotes.map((note: Note) => (
               <NoteCard 
                  ...
                  isPeeking={peekingNoteId === note.id} onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)}
                  ...
                  setHoveredFolderId={setHoveredFolderId} searchQuery={searchQuery}
                  isDraggingGlobal={isDraggingGlobal} setIsDraggingGlobal={setIsDraggingGlobal}
               />
           )) : (
             <>
               {sortedFolders.map((folder: Folder) => (
                  <FolderItem 
                    ...
                    onDeleteNote={onDeleteNote} onMoveNote={onMoveSingleNote}
                    onSettings={() => openFolderSettings(folder.id)} peekingNoteId={peekingNoteId} setPeekingNoteId={setPeekingNoteId}
                    isHovered={hoveredFolderId === folder.id} setHoveredFolderId={setHoveredFolderId}
                    isDraggingGlobal={isDraggingGlobal} setIsDraggingGlobal={setIsDraggingGlobal}
                  />
               ))}
               {sortedRootNotes.map((note: Note) => (
                  <NoteCard 
                    key={note.id} note={note} isDark={isDark} colors={colors} 
                    onClick={() => isSelectionMode ? toggleSelection(note.id) : onOpenNote(note.id)} 
                    ...
                    onMove={(targetFolderId: string | null) => onMoveSingleNote(note.id, targetFolderId)}
                    setHoveredFolderId={setHoveredFolderId} inFolder={false} searchQuery={searchQuery}
        ...
        {isSelectionMode && (
          <motion.div initial={{ y: 100 }} animate={{ y: 0 }} exit={{ y: 100 }} transition={springTransition} className="fixed bottom-0 left-0 right-0 p-4 z-50 flex justify-center pb-8">
             <div className={`flex gap-4 px-6 py-3 rounded-full shadow-2xl ${isDark ? 'bg-zinc-900' : 'bg-white'}`}>
                ...
                {!hasFolderSelected && <button onClick={onMoveClick} className="p-2 rounded-full hover:bg-black/5"><FolderIcon size={24} /></button>}
...
const FolderItem = ({ 
  ...
  peekingNoteId, setPeekingNoteId, isHovered, setHoveredFolderId, isDraggingGlobal, setIsDraggingGlobal
}: any) => {
  const themeClass = colors[folder.color] || colors.slate;
  const handleDragEnd = (event: any, info: PanInfo) => { if (Math.abs(info.offset.x) > 100 && !isSelectionMode) onDelete(); };

  return (
    <motion.div 
       layout transition={springTransition}
       ...
       style={{ zIndex: isExpanded ? 40 : (isHovered ? 10 : 1), position: 'relative' }}
    >
        <div className={`relative w-full rounded-3xl ${isExpanded ? '' : 'h-full'}`}>
            <motion.div
                drag={!isSelectionMode && !isExpanded ? "x" : false} dragConstraints={{ left: 0, right: 0 }} dragElastic={0.5} onDragEnd={handleDragEnd}
                animate={{ scale: (isHovered && !isExpanded) ? 1.05 : 1 }}
                className={`${themeClass} ${folder.shape} relative z-10 w-full ${isExpanded ? 'h-auto min-h-full pb-12' : 'h-full'} ${isSelected ? 'ring-4 ring-indigo-500 ring-offset-2 ring-offset-transparent' : ''} transition-all duration-300`}
            >
                {/* Visual Feedback Overlays */}
                {isHovered && !isExpanded && (
                    <div className="absolute inset-0 z-20 bg-black/20 rounded-[inherit] pointer-events-none flex items-center justify-center">
                        <FolderIcon size={48} className="text-white opacity-80" />
                    </div>
                )}

                {folder.isPinned && <div className="absolute top-4 right-4 text-current opacity-50"><Pin size={16} fill="currentColor"/></div>}
                <div className={`p-5 flex flex-col ${isExpanded ? '' : 'h-full'}`}>
                    <div className="flex justify-between items-start mb-2"><h2 className="text-2xl font-bold leading-tight line-clamp-2 pr-2">{folder.name}</h2> {isExpanded && !isSelectionMode && (<IconButton icon={Palette} onClick={(e: any) => { e.stopPropagation(); onSettings(); }} />)}</div>
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-6 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-4">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    onClick={() => onOpenNote(note.id)} onDelete={() => onMoveNote(note.id, null)} 
                                    isPeeking={peekingNoteId === note.id} onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)}
...
// --- Note Card (FIXED LOGIC) ---
const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove, setHoveredFolderId, searchQuery, onTogglePin, isDraggingGlobal, setIsDraggingGlobal }: any) => {
  ...
  useEffect(() => {
    ...
    const checkAutoScroll = () => {
       ...
       // DEAD ZONE: Cuma scroll kalo di 10% atas atau 10% bawah
       const topZone = 100; 
       const bottomZone = viewportHeight - 100; 
       const maxSpeed = 15;
       ...
       if (y < topZone && y > 0) {
           ...
           const intensity = (y - bottomZone) / 100;
           window.scrollBy(0, maxSpeed * intensity);
       }
       autoScrollInterval.current = requestAnimationFrame(checkAutoScroll);
    };
    autoScrollInterval.current = requestAnimationFrame(checkAutoScroll);
    
    // FIX: BLOCK NATIVE SCROLL saat Dragging
  ...
  const handlePointerDown = (event: React.PointerEvent) => {
     if (isSelectionMode || searchQuery) return;
     // Long Press Trigger
     ...
     }, 250); // Fast but deliberate long press
  };

  const handlePointerMove = (event: React.PointerEvent) => {
      pointerYRef.current = event.clientY;
      if (longPressTimer.current && !isDragging) {
         // Optional: if moved too much, cancel long press? 
         // For now, let's keep it lenient to allow messy fingers
      }
  };
  ...
  // FIX: COLLISION DETECTION (Kotak Tabrakan)
  const handleDragMove = (event: any, info: PanInfo) => {
      ...
      // Ambil kotak elemen yang sedang didrag
      const draggingRect = (event.target as HTMLElement).getBoundingClientRect();
      const dragCenter = {
      ...
      folders.forEach((el) => {
          ...
          // Cek apakah CENTER dragging note ada di dalam kotak folder
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     ...
     if (!wasDragging) { 
         // Swipe Check
         if (Math.abs(info.offset.x) > 100 && !isSelectionMode && !inFolder && !searchQuery) onDelete();
         return;
     }

     // Drop Logic (sama dengan dragMove logic)
     const folders = document.querySelectorAll('[data-folder-id]');
     let foundFolderId: string | null = null;
     const draggingRect = (event.target as HTMLElement).getBoundingClientRect();
     const dragCenter = { x: draggingRect.left + draggingRect.width / 2, y: draggingRect.top + draggingRect.height / 2 };

     folders.forEach((el) => {
         const rect = el.getBoundingClientRect();
         if (dragCenter.x >= rect.left && dragCenter.x <= rect.right && dragCenter.y >= rect.top && dragCenter.y <= rect.bottom) {
             foundFolderId = el.getAttribute('data-folder-id');
         }
     });
     ...
     } else if (!foundFolderId && inFolder) {
         // Drag keluar folder (cukup jauh)
         if (Math.abs(info.offset.y) > 150 || Math.abs(info.offset.x) > 150) onMove(null); 
     }
  };
  ...
  return (
    <motion.div 
       layout layoutId={`note-${note.id}`} transition={springTransition}
    ...
    >
        <motion.div
           ref={cardRef}
           // FIX: Allow FREE DRAG when dragging, otherwise Swipe X only
           drag={isDragging ? true : (inFolder ? false : "x")} 
           dragControls={dragControls}
           dragListener={false} // Kita trigger manual via PointerDown -> Controls.start
           dragDirectionLock={isDragging ? false : true} // LOCK MATI saat dragging supaya bebas
           ...
           animate={{ scale: isDragging ? 1.05 : 1, rotate: isDragging ? 3 : 0, boxShadow: isDragging ? "0px 10px 30px rgba(0,0,0,0.3)" : "none" }}
           className={`w-full h-full relative cursor-pointer group overflow-hidden ${themeClass} ${note.shape} ${isSelected ? 'ring-4 ring-indigo-500 ring-offset-2 ring-offset-transparent' : ''} ${safeIsPeeking ? 'shadow-2xl' : ''} ${isDragging ? 'touch-none' : 'touch-pan-y'}`}
        >
           {note.isPinned && <div className="absolute top-4 left-4 z-20"><Pin size={16} fill="currentColor" className="opacity-70"/></div>}
           <div className="p-5 h-full flex flex-col relative pointer-events-none"> 
               {safeIsPeeking ? (
                   <div className="flex flex-col h-full overflow-hidden">
                       ...
                       <div className="text-lg opacity-80 leading-relaxed overflow-hidden max-h-[300px]">{previewText}</div>
                   </div>
               ) : (
                   <>
                      <h3 className="text-xl font-bold leading-tight line-clamp-2 relative z-10 pt-4">{note.title || "Untitled"}</h3> 
           ...
           {!isSelectionMode && onPeek && !isDragging && (
               <button onPointerDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); onPeek(!safeIsPeeking); }} className={`absolute bottom-3 right-3 p-2 rounded-full shadow-sm z-20 pointer-events-auto transition-transform active:scale-90 ${isDark ? 'bg-zinc-800' : 'bg-white'}`}>{safeIsPeeking ? <Minimize2 size={16}/> : <Maximize2 size={16}/>}</button>
           )}
        </motion.div>
    </motion.div>
  );
