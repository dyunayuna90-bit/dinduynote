...
// --- Constants ---
// Fluid Spring Animation (Lebih mulus, kurang "bouncy" berlebihan)
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={fluidSpring} onClick={e => e.stopPropagation()} className={`w-full max-w-md p-6 rounded-[2rem] shadow-2xl max-h-[85vh] overflow-y-auto ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}>
                ...
                {/* Add Favorite Toggle for Folder */}
                <div className="mb-6">
                    <button onClick={() => onUpdate(folder.id, { isPinned: !folder.isPinned })} className={`w-full p-4 rounded-2xl flex items-center justify-center gap-3 transition-colors ${folder.isPinned ? 'bg-yellow-500/10 text-yellow-600' : 'bg-black/5 hover:bg-black/10'}`}>
                        <Star size={24} fill={folder.isPinned ? "currentColor" : "none"} />
                        <span className="font-bold">{folder.isPinned ? 'Unfavorite Folder' : 'Favorite Folder'}</span>
                    </button>
                </div>
      ...
      {isOpen && (
        <>
          ...
          <motion.div 
            ...
            exit={{ x: '-100%' }} 
            transition={fluidSpring} 
            className={`fixed top-0 left-0 bottom-0 w-[80%] max-w-xs z-[210] shadow-2xl overflow-y-auto flex flex-col ${isDark ? 'bg-[#1a1a1a] text-white' : 'bg-white text-zinc-900'}`}
          >
            ...
            <div className="flex-1 px-4 py-2 space-y-4">
                <a href="https://saweria.co/Densl" target="_blank" rel="noreferrer" className="flex items-center gap-4 p-4 rounded-xl bg-[#fab005] text-black font-bold hover:bg-[#e09e04] transition-colors shadow-lg shadow-orange-500/20">
            ...
            <div className="p-6 text-center opacity-50 text-sm font-medium">
               Crafted with passion in Jakarta
            </div>
          </motion.div>
        </>
      )}
...
// --- Note Editor (FIX: Toolbar Fixed Position) ---
const NoteEditor = ({ note, onUpdate, onClose, onDelete, isDark, showSettings, onCloseSettings, onOpenSettings }: any) => {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex flex-col h-[100dvh] ${themeClass}`}>
       ...
       {/* Editor Area */}
       <div className={`flex-1 overflow-y-auto px-6 py-6 pb-32 bg-transparent`}>
       ...
       <div className={`fixed bottom-0 left-0 right-0 p-2 ${isDark ? 'bg-zinc-900 border-t border-white/10' : 'bg-white border-t border-black/5'} shadow-[0_-4px_20px_rgba(0,0,0,0.1)] z-[110] pb-safe`}>
...
export default function App() {
  ...
  const [suppressDelete, setSuppressDelete] = useLocalStorage<boolean>('desnote-suppress-delete', false); // Persist suppress setting
  ...
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      // 1. Close Peeking Note
      if (peekingNoteId) { setPeekingNoteId(null); }
      // 2. Close Active Note Editor
      else if (activeNoteId) { setActiveNoteId(null); }
      // 3. Close Expanded Folder
      else if (expandedFolderId) { setExpandedFolderId(null); }
      // 4. Close Sidebar/Search/Selection
      else if (showSidebar) { setShowSidebar(false); }
      else if (isSelectionMode) { setIsSelectionMode(false); setSelectedIds(new Set()); }
  ...
  const getFilteredData = () => {
      ...
      if (activeTab === 'favorites') {
          ...
          filteredFolders = filteredFolders.filter(f => f.isPinned); // Folder juga bisa favorit
      ...
      if (searchQuery) {
          if (activeTab === 'folders') {
              filteredFolders = filteredFolders.filter(f => f.name.toLowerCase().includes(searchQuery.toLowerCase()));
          } else if (activeTab === 'notes') {
              filteredNotes = filteredNotes.filter(n => n.title.toLowerCase().includes(searchQuery.toLowerCase()) || n.content.toLowerCase().includes(searchQuery.toLowerCase()));
          } else {
              // All / Favorites / Trash
              filteredNotes = filteredNotes.filter(n => n.title.toLowerCase().includes(searchQuery.toLowerCase()) || n.content.toLowerCase().includes(searchQuery.toLowerCase()));
              filteredFolders = filteredFolders.filter(f => f.name.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const executeDelete = (type: string, id: string | null) => {
      ...
      if (activeTab === 'trash') {
        ...
        if (type === 'note') setNotes(prev => prev.filter(n => n.id !== id));
            ...
            // Notes inside permanently deleted folder also deleted permanently
        ...
        if (type === 'note') setNotes(prev => prev.map(n => n.id === id ? { ...n, isDeleted: true, updatedAt: Date.now() } : n));
             ...
             // Logic: Delete Folder -> Folder Soft Deleted -> Contents Spilled to Dashboard (Requested)
             setNotes(prev => prev.map(n => n.folderId === id ? { ...n, folderId: null, updatedAt: Date.now() } : n));
             setFolders(prev => prev.filter(f => f.id !== id)); // Hapus folder permanen karena isinya tumpah? Atau mau folder masuk sampah?
             // Request: "tiap folder bisa di swipe untuk dihapus, dan ketika folder di swipe... isi folder... auto tumpah semua ke dashboard"
             // Jadi foldernya hilang/masuk sampah, tapi isinya SELAMAT di dashboard.
             // Kita buat foldernya masuk sampah (soft delete) tapi kosong, isinya keluar.
  ...
  // Toggle Favorite for Selection (Folder & Note)
  const toggleFavoriteSelected = () => {
      const ids = Array.from(selectedIds);
      if (ids.length === 0) return;
      
      // Determine toggle direction
      const hasUnpinned = ids.some(id => {
      ...
      setNotes(prev => prev.map(n => ids.includes(n.id) ? { ...n, isPinned: hasUnpinned } : n));
      setFolders(prev => prev.map(f => ids.includes(f.id) ? { ...f, isPinned: hasUnpinned } : f));
      ...
      <AnimatePresence mode="wait">
        {activeNoteId && activeNote ? (
          ...
          <Dashboard 
            ...
            allNotes={notes} // for internal usage inside folder
...
// --- Header Component (COMPACT & FLUID ANIMATION) ---
const Header = ({ 
    ...
    // Animate Header: Compact Height & Elements Scale Down
    const headerHeight = useTransform(scrollY, [0, 100], ['auto', 'auto']); // Keep auto but manipulate inner items
    const titleScale = useTransform(scrollY, [0, 80], [1, 0.7]);
    const titleOpacity = useTransform(scrollY, [0, 80], [1, 0.5]);
    const titleY = useTransform(scrollY, [0, 80], [0, -5]);
    ...
    return (
        <motion.div 
            className={`sticky top-0 z-50 pt-2 px-4 pb-2 transition-colors duration-300 ${isDark ? 'bg-[#121212]/95' : 'bg-[#fdfdfd]/95'} backdrop-blur-md`}
        >
            {/* Top Bar Actions */}
            <div className="flex justify-between items-center mb-1 h-12">
                {isSelectionMode ? (
                     <div className="flex items-center gap-2 w-full">
                        ...
                        {!hasFolderSelected && <button onClick={onMoveClick} className="p-2 rounded-full hover:bg-black/5"><FolderIcon size={22}/></button>}
            ...
            <AnimatePresence>
                {showSearch && !isSelectionMode && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-2">
                         <div className={`flex items-center px-4 py-2 rounded-xl ${isDark ? 'bg-zinc-800' : 'bg-zinc-100'}`}>
                             <input autoFocus value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Type to search..." className="bg-transparent w-full outline-none font-medium text-sm" />
            ...
            {!isSelectionMode && !showSearch && (
                 <motion.div style={{ scale: titleScale, y: titleY, opacity: titleOpacity, originX: 0 }} className="mb-2">
                    ...
                    {tabs.map(tab => {
                        ...
                        return (
                            <motion.button
                                ...
                                initial={false}
                                animate={{ 
                                    width: isActive ? 'auto' : 44, // Lebar lebih compact
                                    backgroundColor: isActive ? (isDark ? '#3f3f46' : '#18181b') : (isDark ? '#27272a' : '#f4f4f5'),
                                    color: isActive ? '#ffffff' : (isDark ? '#a1a1aa' : '#71717a')
                                }}
                                transition={fluidSpring} // Lebih smooth
                                className={`h-10 rounded-full flex items-center justify-center flex-shrink-0 relative overflow-hidden`}
                            >
...
const Dashboard = ({ 
  ...
  // Logic: Dashboard notes
  const dashboardNotes = notes.filter((n: Note) => n.folderId === null);
  const colors = isDark ? DARK_COLORS : LIGHT_COLORS;
...
// --- Folder Item (SWIPE TO DELETE ENABLED + STAR LEFT) ---
...
const FolderItem = ({ 
  ...
  const onDragEnd = (event: any, info: PanInfo) => {
      // Logic: Kalau geser > 100px DAN user melepas, baru hapus.
      // Kalau user tahan (ga lepas), Framer Motion otomatis balikin kalo `dragSnapToOrigin` true,
      // tapi kita butuh trigger delete pas lepas.
      // PanInfo.offset.x -> seberapa jauh digeser.
    ...
    >
        <div className={`relative w-full rounded-3xl ${isExpanded ? '' : 'h-full'}`}>
            <motion.div
                ...
                dragElastic={0.5}
                dragSnapToOrigin={true} // Balik kalau ga jadi hapus
                onDragEnd={onDragEnd}
                ...
                <div className={`p-5 flex flex-col ${isExpanded ? '' : 'h-full'}`}>
                    <div className="flex justify-between items-start mb-2">
                        ...
                        <div className="flex items-center gap-2 pr-2">
                            {folder.isPinned && <Star fill="currentColor" size={14} className="text-black/50" />}
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-4 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-3">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)}
                                    onLongPress={() => {}} // No long press inside expanded for now
                                />
                            ))}
...
// --- Note Card (SWIPE TO DELETE ENABLED + STAR LEFT) ---
               ...
               {safeIsPeeking ? (
                   ...
                   <>
                      <div className="relative z-10 flex items-start gap-1">
                          {note.isPinned && <Star fill="currentColor" size={14} className="text-black/50 mt-1 flex-shrink-0"/>}
                          <h3 className="text-xl font-bold leading-tight line-clamp-2">{note.title || "Untitled"}</h3> 
                      </div>
