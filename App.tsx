...
// --- Modals (Clean Design - No Borders) ---
...
// --- NEW MOVE DIALOG (Corrected Logic) ---
const MoveDialog = ({ isOpen, onClose, onConfirm, folders, isDark }: any) => {
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            ...
            >
               <div className="flex justify-between items-center mb-4 flex-none">
                   <h3 className="text-xl font-bold">Pindahkan ke...</h3>
               ...
               <div className="flex-1 overflow-y-auto space-y-2 mb-4 pr-1">
                  <button 
                    ...
                    className={`w-full p-4 rounded-2xl text-left font-medium flex items-center gap-3 transition-all ${
                        selectedTarget === 'root' 
                        ? 'bg-indigo-500/10 text-indigo-500 ring-2 ring-indigo-500' 
                        : 'hover:bg-black/5'
                    }`}
                  >
                      <CornerDownLeft size={20}/> Dashboard (Home)
                  </button>
               ...
               <div className="flex gap-3 flex-none mt-auto pt-2 border-t border-black/5">
                   ...
                   <button 
                        ...
                        onClick={() => {
                            if (selectedTarget) {
                                onConfirm(selectedTarget === 'root' ? null : selectedTarget);
                            }
                        }} 
                ...
                <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-3">
                        {Object.keys(colors).map(c => {
                             ...
                             return (
                                <button key={c} onClick={() => onUpdate(note.id, {color: c})} className={`w-10 h-10 rounded-full ${bgClass} ${isActive ? 'ring-4 ring-offset-2 ring-indigo-500 dark:ring-offset-zinc-900' : ''}`} />
                             )
                        })}
                    </div>
                </div>
                <div className="mb-6">
                    ...
                    <div className="grid grid-cols-4 gap-3">
                        {SHAPES.slice(0, 12).map((s,i) => (
                            <button key={i} onClick={() => onUpdate(note.id, {shape: s})} className={`h-12 bg-current opacity-10 hover:opacity-30 transition-opacity ${s} ${note.shape === s ? '!opacity-100 ring-2 ring-indigo-500' : ''}`} />
                        ))}
                    </div>
                </div>
                <div className="mb-6">
                    ...
                    <div className="grid grid-cols-6 gap-2">
                        {Object.keys(ICONS).map(k => { 
                            const I = ICONS[k]; 
                            return (
                                <button key={k} onClick={() => onUpdate(note.id, {icon: k})} className={`p-2 hover:bg-black/5 rounded flex items-center justify-center ${note.icon === k ? 'text-indigo-600 bg-indigo-50' : ''}`}><I size={20}/></button>
                            ) 
                        })}
                    </div>
                </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            ...
            >
                ...
                <div className="mb-6">
                    ...
                    <input value={folder.name} onChange={(e) => onUpdate(folder.id, { name: e.target.value })} className={`w-full p-3 rounded-xl text-lg font-medium outline-none ${isDark ? 'bg-zinc-800 focus:bg-zinc-700' : 'bg-zinc-100 focus:bg-zinc-200'}`} />
                </div>
                <div className="mb-6">
                    <h4 className="text-xs font-bold opacity-50 mb-3 uppercase tracking-wider">Color Theme</h4>
                    <div className="flex flex-wrap gap-3">
                        {Object.keys(colors).map(c => {
                             ...
                             return (
                                <button key={c} onClick={() => onUpdate(folder.id, {color: c})} className={`w-10 h-10 rounded-full ${bgClass} ${isActive ? 'ring-4 ring-offset-2 ring-indigo-500 dark:ring-offset-zinc-900' : ''}`} />
                             )
                        })}
                    </div>
                </div>
                <div>
                    ...
                    <div className="grid grid-cols-4 gap-3">
                        {SHAPES.slice(0, 12).map((s,i) => (
                            <button key={i} onClick={() => onUpdate(folder.id, {shape: s})} className={`h-12 bg-current opacity-10 hover:opacity-30 transition-opacity ${s} ${folder.shape === s ? '!opacity-100 ring-2 ring-indigo-500' : ''}`} />
                        ))}
                    </div>
                </div>
            </motion.div>
...
export default function App() {
  ...
  const executeDelete = (type: string, id: string | null) => {
      ...
      else if (type === 'folder') { setNotes(prev => prev.map(n => n.folderId === id ? { ...n, folderId: null } : n)); setFolders(prev => prev.filter(f => f.id !== id)); if (expandedFolderId === id) setExpandedFolderId(null); }
  };

  const requestDelete = (type: string, id: string) => {
      if (suppressDelete) { executeDelete(type, id); } else { setDeleteModal({ isOpen: true, type, id }); }
  };

  // --- LOGIC MOVE SELECTED (FIXED WITH ALERT) ---
  const handleMoveSelected = (targetId: string | null) => {
      const idsToMove = Array.from(selectedIds);
      if (idsToMove.length === 0) return;

      // DEBUG ALERT (Supaya tau kodingan ini jalan)
      alert(`Berhasil memindahkan ${idsToMove.length} catatan!`);

      setNotes(prevNotes => {
          return prevNotes.map(n => {
              if (idsToMove.includes(n.id)) {
                  // Explicitly return new object
                  return { ...n, folderId: targetId, updatedAt: Date.now() }; 
              }
              return n;
          });
      });
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-[#121212] text-[#f0f0f0]' : 'bg-[#fdfdfd] text-[#1a1c1e]'}`}>
      ...
      <DeleteConfirmationModal isOpen={deleteModal.isOpen} onClose={() => setDeleteModal({...deleteModal, isOpen: false})} isDark={isDark} onConfirm={(suppress: boolean) => { if (suppress) setSuppressDelete(true); executeDelete(deleteModal.type, deleteModal.id); setDeleteModal({...deleteModal, isOpen: false}); }} />
      ...
      {/* Move Dialog (Di Level App) */}
      <MoveDialog isOpen={showMoveDialog} onClose={() => setShowMoveDialog(false)} onConfirm={handleMoveSelected} folders={folders} isDark={isDark} />
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          ...
          <Dashboard 
            ...
            onDeleteFolder={(id) => requestDelete('folder', id)} onDeleteNote={(id) => requestDelete('note', id)}
             ...
             <>
               {folders.map((folder: Folder) => (
                  <FolderItem 
                    ...
                    onDeleteNote={onDeleteNote} onSettings={() => openFolderSettings(folder.id)}
...
const FolderItem = ({ folder, notes, isExpanded, isDark, colors, onToggle, onClose, onOpenNote, onDelete, onUpdate, onCreateNote, isSelectionMode, isSelected, onSelect, onDeleteNote, onSettings, peekingNoteId, setPeekingNoteId }: any) => {
  ...
  return (
    <motion.div layout transition={springTransition} className={`${isExpanded ? 'col-span-full' : 'col-span-1 aspect-square'}`} onClick={(e) => { e.stopPropagation(); if (isSelectionMode) onSelect(); else isExpanded ? onClose() : onToggle(); }}>
        <div className="relative w-full h-full rounded-3xl">
            <motion.div drag={!isExpanded && !isSelectionMode ? "x" : false} dragConstraints={{ left: 0, right: 0 }} dragElastic={{ left: 0.5, right: 0.5 }} onDragEnd={handleDragEnd} className={`${themeClass} ${folder.shape} relative z-10 w-full h-full ${isSelected ? 'ring-4 ring-indigo-500 ring-offset-2 ring-offset-transparent' : ''}`}>
                <div className="p-6">
                    <div className="flex justify-between items-start"><h2 className="text-2xl font-bold truncate pr-2">{folder.name}</h2>{isExpanded && !isSelectionMode && (<IconButton icon={Palette} onClick={(e: any) => { e.stopPropagation(); onSettings(); }} />)}</div>
                ...
                <AnimatePresence>
                    {isExpanded && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-6 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-4">
                            {notes.map((note: Note) => (
                                <NoteCard key={note.id} note={note} isDark={isDark} colors={colors} inFolder onClick={() => onOpenNote(note.id)} onDelete={() => onDeleteNote(note.id)} isPeeking={peekingNoteId === note.id} onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)} />
                            ))}
...
const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek }: any) => {
  ...
  return (
    <motion.div layout transition={springTransition} className={`${safeIsPeeking ? 'col-span-2 row-span-2 z-30' : 'col-span-1 aspect-square'} relative`} onClick={(e) => { e.stopPropagation(); onClick(); }}>
        <div className="relative w-full h-full">
            <motion.div drag={!safeIsPeeking && !isSelectionMode ? "x" : false} dragConstraints={{ left: 0, right: 0 }} dragElastic={{ left: 0.5, right: 0.5 }} onDragEnd={handleDragEnd} layoutId={`note-${note.id}`} transition={springTransition} className={`w-full h-full relative z-10 cursor-pointer group overflow-hidden ${themeClass} ${note.shape} ${isSelected ? 'ring-4 ring-indigo-500 ring-offset-2 ring-offset-transparent' : ''} ${safeIsPeeking ? 'shadow-2xl' : ''}`}>
               <div className="p-6 h-full flex flex-col relative">
                   {safeIsPeeking ? (
                       <div className="flex flex-col h-full overflow-hidden"><h3 className="text-3xl font-bold mb-4 leading-tight">{note.title || "Untitled"}</h3><div className="text-lg opacity-80 leading-relaxed overflow-hidden max-h-[300px]">{previewText}</div></div>
                   ) : (
                       <><h3 className="text-xl font-bold leading-tight line-clamp-3 relative z-10">{note.title || "Untitled"}</h3><div className="absolute bottom-4 right-4 opacity-10 pointer-events-none"><Icon size={80} /></div></>
                   )}
                   {!isSelectionMode && onPeek && (<button onClick={(e) => { e.stopPropagation(); onPeek(!safeIsPeeking); }} className={`absolute bottom-3 right-3 p-2 rounded-full shadow-sm z-20 transition-transform active:scale-90 ${isDark ? 'bg-zinc-800' : 'bg-white'}`}>{safeIsPeeking ? <Minimize2 size={16}/> : <Maximize2 size={16}/>}</button>)}
