...
// Hook untuk menghitung tinggi layar asli (mengatasi masalah keyboard HP)
const useVisualViewport = () => {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      // visualViewport.height memberikan tinggi 'nyata' yang terlihat (di atas keyboard)
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark, showSettings, onCloseSettings, onOpenSettings }: any) => {
  ...
  // FIX 4: Gunakan visual viewport height agar toolbar tidak ketutupan keyboard
  ...
  return (
    // FIX 4: Gunakan style height manual dari visualViewport
    <div 
        className={`fixed top-0 left-0 w-full z-[100] flex flex-col ${themeClass}`}
             ...
             <>
               {folders.map((folder: Folder) => (
                  <FolderItem 
                    ...
                    // FIX 1 & 2: Pass down setHoveredFolderId to children notes
                    setHoveredFolderId={setHoveredFolderId}
                  />
               ))}
...
const FolderItem = ({ 
  ...
  return (
    <motion.div 
       ...
       // FIX 1 & 2: Z-Index harus tinggi saat Expanded agar tidak tertutup folder di bawahnya
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-6 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-4">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    // FIX 1: Pass setHovered agar note di dalam folder bisa trigger hover folder lain
                                    setHoveredFolderId={setHoveredFolderId}
                                />
                            ))}
...
// --- Note Card (FIX FINAL: SCROLL CONFLICT & Z-INDEX) ---
...
const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove, setHoveredFolderId, searchQuery }: any) => {
  ...
  const handlePointerMove = (event: React.PointerEvent) => {
      ...
      if (pointerStartPosition.current) {
          ...
          // Cancel long press if moved significantly before drag starts (Scroll detection)
  ...
  const handleDragMove = (event: any, info: PanInfo) => {
      ...
      // Logika deteksi folder yang lebih akurat
      folders.forEach((el) => {
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     if (!isDragging) {
         if (Math.abs(info.offset.x) > 100 && !isSelectionMode && !inFolder) {
             onDelete();
         }
         return;
     }
  ...
  return (
    <motion.div 
       ...
       transition={springTransition}
       // FIX 2: Z-Index SUPER TINGGI saat dragging supaya tidak ketutupan folder lain
       style={{ zIndex: isDragging ? 100 : (safeIsPeeking ? 50 : 1), position: 'relative' }}
    ...
    >
        <motion.div
           ref={cardRef}
           // FIX 3: Disable swipe X jika long press belum aktif, biar scroll lancar
