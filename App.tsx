...
const stiffTransition = { type: "tween", ease: "easeInOut", duration: 0.2 }; // Kaku & Cepet buat Sidebar
      ...
      {isOpen && (
        <>
          ...
          <motion.div 
            ...
            animate={{ x: 0 }} 
            exit={{ x: '-100%' }} 
            transition={stiffTransition} // USE STIFF TRANSITION
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, isDark, showSettings, onCloseSettings, onOpenSettings }: any) => {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex flex-col h-[100dvh] ${themeClass}`}>
       ...
       {/* Top Bar - Fixed */}
       <div className={`flex-none flex justify-between items-center p-4 bg-transparent z-[10]`}>
       ...
       <div className={`flex-1 overflow-y-auto px-6 py-6 bg-transparent`}>
          <div className="max-w-4xl mx-auto pb-[150px]"> {/* Add Huge Padding Bottom */}
       ...
       {/* Toolbar - ABSOLUTELY FIXED AT BOTTOM */}
       <div className="flex-none z-[120] w-full bg-inherit shadow-[0_-4px_20px_rgba(0,0,0,0.1)] pb-safe-area-bottom">
          <div className={`p-2 w-full ${isDark ? 'bg-zinc-900' : 'bg-white'}`}>
...
export default function App() {
  ...
  // --- Satpam Back Button Logic (STRICT HIERARCHY FIX) ---
  useEffect(() => {
    // Helper to push state safely
    const pushState = () => {
       window.history.pushState(null, '', window.location.href);
    };

    // When we enter a state that captures back button, we push a new history entry.
    // This allows the "pop" to be captured by our handler.
    if (activeNoteId || expandedFolderId || peekingNoteId || showSidebar || isSelectionMode) {
        // We only push if we are transitioning INTO a modal state.
        // However, React effect runs on every change. 
        // We need to be careful not to infinite loop history.
        // Simplified: Just ensure we have history entries to pop.
        pushState();
    }
    
    const handlePopState = (event: PopStateEvent) => {
      // 1. Modals & Dialogs (Highest Priority)
      ...
      // 2. Editor (Full Screen)
      if (activeNoteId) { setActiveNoteId(null); return; }

      // 3. Peeking Note (Must close BEFORE Folder)
      if (peekingNoteId) { 
          setPeekingNoteId(null); 
          // CRITICAL: If folder is expanded, we are NOT done. 
          // We consumed the "Back" event for Peeking.
          // The browser history popped 1.
          // We are now at state where Folder is Expanded.
          // The next back press will handle Folder.
          return; 
      }
  ...
  const openNoteFromPeek = (noteId: string) => {
      // Logic: Transition from Peek to Full.
      // We must clean up Peek ID but NOT close Folder.
      // And we must add history stack for Full Note.
      setPeekingNoteId(null); // Close peek
      setActiveNoteId(noteId); // Open full
  };
      ...
      <AnimatePresence mode="wait">
        {activeNoteId && activeNote ? (
          ...
          <Dashboard 
            ...
            onOpenNote={openNoteFromPeek} // Use safe transition
