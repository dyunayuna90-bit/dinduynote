...
// --- MOVE DIALOG (YANG UDAH DIPERBAIKI) ---
...
const NoteSettingsModal = ({ isOpen, onClose, note, onUpdate, onMoveNote, folders, isDark, colors }: any) => {
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} onClick={e => e.stopPropagation()} className={`w-full max-w-md p-6 rounded-[2rem] shadow-2xl max-h-[85vh] overflow-y-auto ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}>
                ...
                <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-3">
                        {Object.keys(colors).map(c => {
                             ...
                             return <button key={c} onClick={() => onUpdate(note.id, {color: c})} className={`w-10 h-10 rounded-full ${bgClass} ${isActive ? 'ring-4 ring-offset-2 ring-indigo-500 dark:ring-offset-zinc-900' : ''}`} />
                        })}
                    </div>
                </div>
                <div className="mb-6">
                    ...
                    <div className="grid grid-cols-4 gap-3">
                        {SHAPES.slice(0, 12).map((s,i) => <button key={i} onClick={() => onUpdate(note.id, {shape: s})} className={`h-12 bg-current opacity-10 hover:opacity-30 transition-opacity ${s} ${note.shape === s ? '!opacity-100 ring-2 ring-indigo-500' : ''}`} />)}
                    </div>
                </div>
                <div className="mb-6">
                    <h4 className="text-xs font-bold opacity-50 mb-3 uppercase tracking-wider">Icon</h4>
                    <div className="grid grid-cols-6 gap-2">
                        {Object.keys(ICONS).map(k => { const I = ICONS[k]; return <button key={k} onClick={() => onUpdate(note.id, {icon: k})} className={`p-2 hover:bg-black/5 rounded flex items-center justify-center ${note.icon === k ? 'text-indigo-600 bg-indigo-50' : ''}`}><I size={20}/></button> })}
                    </div>
                </div>
                <div>
                    <h4 className="text-xs font-bold opacity-50 mb-3 uppercase tracking-wider">Location</h4>
                    <div className="flex flex-col gap-1">
                        <button onClick={() => onMoveNote(note.id, null)} className={`w-full text-left p-3 rounded-xl font-medium flex items-center gap-3 transition-colors ${note.folderId === null ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300' : 'hover:bg-black/5'}`}><CornerDownLeft size={18}/> Dashboard</button>
                        {folders.map((f: Folder) => <button key={f.id} onClick={() => onMoveNote(note.id, f.id)} className={`w-full text-left p-3 rounded-xl font-medium flex items-center gap-3 transition-colors ${note.folderId === f.id ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300' : 'hover:bg-black/5'}`}><FolderIcon size={18}/> {f.name}</button>)}
                    </div>
                </div>
            </motion.div>
        </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} onClick={e => e.stopPropagation()} className={`w-full max-w-md p-6 rounded-[2rem] shadow-2xl max-h-[85vh] overflow-y-auto ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}>
                ...
                <div>
                    <h4 className="text-xs font-bold opacity-50 mb-3 uppercase tracking-wider">Shape</h4>
                    <div className="grid grid-cols-4 gap-3">
                        {SHAPES.slice(0, 12).map((s,i) => <button key={i} onClick={() => onUpdate(folder.id, {shape: s})} className={`h-12 bg-current opacity-10 hover:opacity-30 transition-opacity ${s} ${folder.shape === s ? '!opacity-100 ring-2 ring-indigo-500' : ''}`} />)}
                    </div>
                </div>
            </motion.div>
        </div>
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'tween', duration: 0.3 }} className={`fixed top-0 left-0 bottom-0 w-80 z-[70] p-6 shadow-2xl overflow-y-auto ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}>
            ...
            <div className={`p-5 rounded-2xl mb-6 ${isDark ? 'bg-zinc-800' : 'bg-zinc-100'}`}>
               ...
               <a href="https://saweria.co/Densl" target="_blank" rel="noopener noreferrer" className="w-full py-3 rounded-xl font-medium bg-[#fab005] text-black hover:bg-[#e09e04] transition-colors flex items-center justify-center gap-2"><Heart size={18} fill="black" /> Donate (Saweria)</a>
            </div>
            <div className="space-y-4">
              <button onClick={toggleTheme} className={`w-full flex items-center gap-3 p-4 rounded-xl transition-colors ${isDark ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-zinc-100 hover:bg-zinc-200'}`}><Sun className="text-yellow-400" /><span className="font-medium">{isDark ? 'Light Mode' : 'Dark Mode'}</span></button>
...
export default function App() {
  ...
  // --- STAGING STATE (KUNCI DATA DISINI) ---
  ...
  // --- Satpam Back Button (TANPA CLEAR SELECTION) ---
  ...
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      ...
      // NOTE: Saya hapus logic clear selection saat back, biar aman dulu.
  ...
  // --- PREPARE MOVE (KUNCI DATA SEBELUM DIALOG BUKA) ---
  const prepareMove = () => {
      const ids = Array.from(selectedIds);
      if (ids.length === 0) return;
      setItemsToMove(ids); // Simpan ke state aman
      setShowMoveDialog(true);
  };

  // --- EXECUTE MOVE (BACA DARI STATE AMAN) ---
  const handleMoveSelected = (targetId: string | null) => {
      if (itemsToMove.length === 0) return;

      setNotes(prevNotes => {
          return prevNotes.map(n => {
              if (itemsToMove.includes(n.id)) {
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-[#121212] text-[#f0f0f0]' : 'bg-[#fdfdfd] text-[#1a1c1e]'}`}>
      ...
      {/* Pass itemsToMove.length to Dialog */}
      <MoveDialog 
         ...
         folders={folders} 
         isDark={isDark}
         selectedCount={itemsToMove.length} 
      />
