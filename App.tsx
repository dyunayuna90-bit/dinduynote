...
import { 
  ...
  Undo, Redo, Pin, PinOff // NEW: Import Pin Icons
} from 'lucide-react';
import { v4 as uuidv4 } from 'uuid';
import { Note, Folder, Shape, ColorTheme } from './types';
import { DARK_COLORS, LIGHT_COLORS, SHAPES, ICONS } from './constants';

// --- Constants (PERMULUS ANIMASI DI SINI) ---

// Stiffness tinggi + damping pas = animasi cepet tapi gak mantul lebay (Snappy)
const springTransition = { type: "spring", stiffness: 450, damping: 35, mass: 0.8 };
...
// NEW: Hook buat Long Press (Tahan Lama)
const useLongPress = (callback: () => void, ms = 500) => {
  const timeoutRef = useRef<NodeJS.Timeout | null>(null);
  const targetRef = useRef<EventTarget | null>(null);

  const start = useCallback((event: React.MouseEvent | React.TouchEvent) => {
      // Prevent context menu on long press usually
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} onClick={e => e.stopPropagation()} className={`w-full max-w-md p-6 rounded-[2rem] shadow-2xl max-h-[85vh] overflow-y-auto ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}>
                ...
                {/* NEW: Pin Option */}
                <div className="mb-6">
                  <h4 className="text-xs font-bold opacity-50 mb-3 uppercase tracking-wider">Actions</h4>
                  <button onClick={() => onUpdate(note.id, { pinned: !note.pinned })} className={`w-full text-left p-3 rounded-xl font-medium flex items-center gap-3 transition-colors ${note.pinned ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300' : 'hover:bg-black/5'}`}>
                ...
                <div><h4 className="text-xs font-bold opacity-50 mb-3 uppercase tracking-wider">Location</h4><div className="flex flex-col gap-1"><button onClick={() => onMoveNote(note.id, null)} className={`w-full text-left p-3 rounded-xl font-medium flex items-center gap-3 transition-colors ${note.folderId === null ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300' : 'hover:bg-black/5'}`}><CornerDownLeft size={18}/> Dashboard</button>{folders.map((f: Folder) => <button key={f.id} onClick={() => onMoveNote(note.id, f.id)} className={`w-full text-left p-3 rounded-xl font-medium flex items-center gap-3 transition-colors ${note.folderId === f.id ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300' : 'hover:bg-black/5'}`}><FolderIcon size={18}/> {f.name}</button>)}</div></div>
            </motion.div>
        </div>
    );
};
...
// --- UPDATED SIDEBAR: BAHASA INGGRIS ---
    ...
    <div className={`fixed inset-0 z-[100] flex flex-col ${themeClass}`}>
       ...
       <div className={`flex-none flex justify-between items-center p-4 bg-transparent`}>
          ...
          <div className="flex gap-2 relative">
             {note.pinned && <div className="flex items-center justify-center p-3 opacity-50"><Pin size={20} fill="currentColor" /></div>}
...
export default function App() {
  ...
  const createNote = (folderId: string | null = null) => {
    const newNote: Note = { id: uuidv4(), title: '', content: '', folderId, color: 'slate', shape: 'rounded-tl-[2.5rem] rounded-br-[2.5rem] rounded-tr-xl rounded-bl-xl', icon: 'file-text', updatedAt: Date.now(), pinned: false };
  ...
  const handleMoveSelected = (targetId: string | null) => {
      ...
      alert(`Successfully moved ${itemsToMove.length} items!`);
  };

  const ejectNote = (id: string) => {
      setNotes(prev => prev.map(n => n.id === id ? { ...n, folderId: null, updatedAt: Date.now() } : n));
  };
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          ...
          <Dashboard 
            ...
            onDeleteFolder={(id) => requestDelete('folder', id, "Folder will be deleted, notes inside will move to Dashboard.")} 
            onDeleteNote={(id) => requestDelete('note', id)}
            onEjectNote={ejectNote} 
...
const Dashboard = ({ 
  ...
  onOpenNote, onUpdateFolder, onDeleteFolder, onDeleteNote, onCreateNoteInFolder, onEjectNote,
  ...
  // Logic sorting PIN: Pinned notes always first
  const rootNotes = notes
    .filter((n: Note) => n.folderId === null)
    .sort((a: Note, b: Note) => (b.pinned === a.pinned ? 0 : b.pinned ? 1 : -1));
        ...
        <motion.div layout transition={springTransition} className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
           {searchQuery ? filteredNotes.map((note: Note) => (
               <NoteCard 
                  ...
                  onLongPress={() => { setIsSelectionMode(true); toggleSelection(note.id); }} 
             ...
             <>
               {folders.map((folder: Folder) => (
                  <FolderItem 
                    key={folder.id} folder={folder} notes={notes.filter((n: Note) => n.folderId === folder.id).sort((a: Note, b: Note) => (b.pinned === a.pinned ? 0 : b.pinned ? 1 : -1))}
                    ...
                    onLongPress={() => { setIsSelectionMode(true); toggleSelection(folder.id); }}
                    onDeleteNote={onEjectNote} 
                    onSettings={() => openFolderSettings(folder.id)}
                    peekingNoteId={peekingNoteId} setPeekingNoteId={setPeekingNoteId}
                    // Pass Toggle Selection logic to folder to handle children selection
...
const FolderItem = ({ 
  ...
  peekingNoteId, setPeekingNoteId, onLongPress, toggleSelection, selectedIds
}: any) => {
  const themeClass = colors[folder.color] || colors.slate;

  // Bind Long Press logic
  const longPressHandlers = useLongPress(() => {
     if (!isExpanded && onLongPress) onLongPress();
  });
  ...
  return (
    <motion.div 
       ...
       {...longPressHandlers} // Apply Long Press
    >
        <div className={`relative w-full rounded-3xl ${isExpanded ? '' : 'h-full'}`}>
            <motion.div
                ...
                className={`${themeClass} ${folder.shape} relative z-10 w-full ${isExpanded ? 'h-auto min-h-full' : 'h-full'} ${isSelected ? 'ring-4 ring-indigo-500 ring-offset-2 ring-offset-transparent' : ''}`}
            >
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-6 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-4">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    // FIX: Check selection mode inside expanded folder
                                    onClick={() => isSelectionMode ? toggleSelection(note.id) : onOpenNote(note.id)} 
                                    onLongPress={() => { toggleSelection(note.id); }} // Long press inside folder just toggles selection
                                    onDelete={() => onDeleteNote(note.id)} 
                                    isPeeking={peekingNoteId === note.id}
                                    onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)}
...
const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onLongPress }: any) => {
               ...
               <div className="p-5 h-full flex flex-col relative">
                   ...
                   {safeIsPeeking ? (
                       <div className="flex flex-col h-full overflow-hidden">
                           <div className="flex justify-between items-start mb-2">
                             <h3 className="text-3xl font-bold leading-tight">{note.title || "Untitled"}</h3>
                             {note.pinned && <Pin size={24} fill="currentColor" className="opacity-60 flex-shrink-0" />}
                           </div>
                       ...
                       <>
                          <h3 className="text-xl font-bold leading-tight line-clamp-2 relative z-10 pr-4">{note.title || "Untitled"}</h3>
