...
// --- Note Editor (FIX 1: KEYBOARD FLICKER/JITTER) ---
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark, showSettings, onCloseSettings, onOpenSettings }: any) => {
  ...
  // SOLUSI BUG 1: Hapus kalkulasi JS yang berat (useVisualViewport).
  // Gunakan 100dvh (Dynamic Viewport Height) yang support native mobile browser behavior.
  // 'fixed inset-0' memastikan element nempel di layar.
  ...
  return (
    <div 
        className={`fixed inset-0 z-[100] flex flex-col ${themeClass} h-[100dvh]`} // Gunakan 100dvh
    >
       ...
       {/* Toolbar - FIXED at bottom. 
           Di mobile, browser modern akan otomatis mendorong bottom ke atas keyboard.
       */}
    ...
    >
        <div className={`relative w-full rounded-3xl ${isExpanded ? '' : 'h-full'}`}>
            <motion.div
                ...
                // SOLUSI BUG 3: Indikator Expand. 
                // Jika isHovered (ada note di atasnya), scale up sedikit.
...
// --- Note Card (FIX: AUTO SCROLL & VERTICAL DRAG LOCK) ---
...
const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove, setHoveredFolderId, searchQuery, onTogglePin }: any) => {
  ...
  useEffect(() => {
    ...
    const checkAutoScroll = () => {
       ...
       const topZone = 100; // px dari atas
       const bottomZone = viewportHeight - 100; // px dari bawah
       const speed = 15;
    ...
    // FIX 4: Block Browser Scroll completely when dragging
  ...
  const handlePointerMove = (event: React.PointerEvent) => {
      // Update pointer Y for auto-scroll
      pointerYRef.current = event.clientY;

      // Cancel long press if moved significantly before drag starts (prevent accidental drag while scrolling)
      // but only if NOT already dragging
      if (longPressTimer.current && !isDragging) {
         // Logic optional: if distance > 10px, clear timeout
      }
  };
  ...
  const handleDragMove = (event: any, info: PanInfo) => {
      // Update pointer Y for auto-scroll from Drag info as fallback
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     ...
     if (foundFolderId && foundFolderId !== note.folderId) {
         ...
         // Dragged OUT of folder (if moved far enough vertically)
  ...
  return (
    ...
    >
        <motion.div
           ref={cardRef}
           // FIX 4: Explicit Drag Direction Lock False
           // Ketika isDragging aktif, drag bebas (true). 
           // Ketika tidak, hanya 'x' (swipe).
           drag={isDragging ? true : (inFolder ? false : "x")} 
           dragControls={dragControls}
           dragListener={true} 
           
           // FIX 4: Matikan lock direction saat dragging dimulai
           dragDirectionLock={isDragging ? false : true} 
           ...
           // Touch-action diatur dinamis via useEffect di atas (document.body), 
           // tapi tetap set disini sebagai fallback
           className={`
