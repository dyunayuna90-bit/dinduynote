...
import { 
  ...
  Undo, Redo, Pin, PinOff, Star, RefreshCcw, Layers, FileText, Share2
} from 'lucide-react';
...
// JURUS RAHASIA DARI V 1.7 (Visual Viewport Hook)
    ...
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="opacity-60 text-sm">{message || "Item ini akan dihapus."}</p>
        </div>
        <div className="flex items-center gap-2 mb-6 justify-center opacity-70 cursor-pointer" onClick={() => setSuppress(!suppress)}>
           ...
           <span className="text-xs font-medium">Jangan ingatkan lagi</span>
        </div>
...
const Sidebar = ({ isOpen, onClose, isDark, toggleTheme, onExport, onImport }: any) => {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 bg-black/40 z-[200]" />
          <motion.div 
            initial={{ x: '-100%' }} 
            animate={{ x: 0 }} 
            exit={{ x: '-100%' }} 
            transition={{ type: 'tween', duration: 0.25, ease: "easeOut" }} // KAKU tapi SAT-SET
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, isDark, showSettings, onCloseSettings, onOpenSettings }: any) => {
  ...
  return (
    // FIX UTAMA: Pake Fixed Position dengan kalkulasi Viewport Real-time
    <div 
    ...
    >
          ...
          <div className="max-w-4xl mx-auto pb-4">
              ...
              <div 
                  ...
                  onInput={(e) => { 
                      ...
                      checkFormats(); 
                      scrollToCursor(); // Auto Scroll jalan karena viewport udah bener
                  }} 
       ...
       {/* Toolbar - Nempel di bawah karena Flex-col + Height Viewport */}
...
export default function App() {
  ...
  // --- Satpam Back Button Logic (STRICT & HISTORY POPPED REF) ---
  const historyPopped = useRef(false);

  useEffect(() => {
    // Push state hanya jika belum di-pop secara manual dan ada perubahan UI modal
    if (!historyPopped.current) {
        if (activeNoteId || expandedFolderId || peekingNoteId || showSidebar || isSelectionMode || showNoteSettings || folderSettingsId || showMoveDialog || deleteModal.isOpen) {
            window.history.pushState(null, '', window.location.href);
        }
    }
    historyPopped.current = false; // Reset flag

    const handlePopState = (event: PopStateEvent) => {
      historyPopped.current = true; // Tandai bahwa event ini dari tombol back browser
      ...
      // 2. Peeking Note (HARUS DULUAN DARI FOLDER)
      if (peekingNoteId) { 
          setPeekingNoteId(null); 
          // Stop disini. Jangan tutup folder.
          // Browser history udah mundur 1 step (buat tutup peeking).
          // Step berikutnya di history stack adalah state dimana Folder Expanded.
          return; 
      }
      ...
      if (activeTab === 'trash') {
        ...
        if (type === 'note') {
            setNotes(prev => prev.map(n => n.id === id ? { ...n, isDeleted: true, updatedAt: Date.now() } : n));
             ...
             // Logic Muntah: Isi folder keluar ke dashboard
...
const Header = ({ 
    ...
    const getHeaderTitle = () => {
        ...
        if (activeTab === 'favorites') return "Favorit";
        if (activeTab === 'folders') return "Folder";
        if (activeTab === 'notes') return "Catatan";
        if (activeTab === 'trash') return "Sampah";
        return "Desnote";
    };

    return (
        <div className={`sticky top-0 z-50 px-4 pt-4 pb-2 transition-colors duration-300 ${isDark ? 'bg-[#121212]/95' : 'bg-[#fdfdfd]/95'} backdrop-blur-md`}>
...
const FolderItem = ({ 
  ...
  // Ref buat nyimpen posisi awal drag biar tau ini TAP apa SWIPE
  ...
  const onPanStart = () => {
      // Kalau user mulai geser (swipe), cancel long press!
      if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; }
  }

  const onDragEnd = (event: any, info: PanInfo) => {
      // Logic swipe delete: Cuma kalo geser jauh (>100px)
  ...
  return (
    <motion.div 
       ...
       onClick={(e) => { 
           e.stopPropagation(); 
           // Logic anti-swipe click: Cek geser ga? Kalo geser dikit, anggep click.
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-4 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-3">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)}
                                    // Fix: Select dalam folder berfungsi!
...
const NoteCard = ({ note, onClick, onDelete, onRestore, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, searchQuery, isTrash, onLongPress }: any) => {
  ...
  const handlePointerDown = () => {
      // Fix: Kalo lagi peeking, ga usah long press
  ...
  const onPanStart = () => {
      // Fix: Swipe ke-detect? Batalin select!
      if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; }
  }
