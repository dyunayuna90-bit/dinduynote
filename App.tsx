...
const FolderItem = ({ 
  ...
  return (
    <motion.div 
       ...
       // ID ini PENTING untuk dideteksi oleh NoteCard
       data-folder-id={folder.id} 
       style={{ zIndex: isHovered ? 10 : 1 }} // Folder naik sedikit z-indexnya saat di-hover
    >
        <div className={`relative w-full rounded-3xl ${isExpanded ? '' : 'h-full'}`}>
            <motion.div
                // ANIMASI MEMBESAR KETIKA NOTE BERADA DI ATASNYA (Visual Feedback)
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-6 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-4">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    onDelete={() => onMoveNote(note.id, null)} // Eject
                                    isPeeking={peekingNoteId === note.id}
                                    onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)}
                                    onMove={(target: string | null) => onMoveNote(note.id, target)}
                                    // di dalam folder tidak perlu setHoveredFolderId karena tidak bisa drag keluar
                                />
                            ))}
...
// --- Note Card (FIX LENGKAP - DRAG & DROP) ---
...
const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove, setHoveredFolderId }: any) => {
  ...
  const dragControls = useDragControls(); // Hook wajib untuk manual control
  const cardRef = useRef<HTMLDivElement>(null);
  const longPressTimer = useRef<NodeJS.Timeout | null>(null);

  // Fungsi saat Long Press terjadi
  const startDragSequence = (event: any) => {
      // 1. Getar
      if (navigator.vibrate) navigator.vibrate(50);
      
      // 2. Set State Dragging
      setIsDragging(true);

      // 3. MULAI DRAG SECARA MANUAL (KUNCI UTAMA FIX)
      // Kita snapToCursor agar note langsung nempel di jari/mouse
      dragControls.start(event, { snapToCursor: true });
  };

  const handlePointerDown = (event: React.PointerEvent) => {
     if (isSelectionMode) return;
     // Jangan drag jika di dalam folder (kecuali Anda mau fitur itu)
     if (inFolder) return; 
     ...
     if (safeIsPeeking) {
         // LOGIKA KHUSUS PEEKING:
         // 1. Kecilkan dulu (exit peek)
         onPeek(false);
         // 2. Tunggu sebentar (100ms) agar layout stabil, baru mulai drag
         longPressTimer.current = setTimeout(() => {
             startDragSequence(event);
         }, 100);
     } else {
         // LOGIKA NORMAL:
         // Tahan 250ms untuk mulai drag
         longPressTimer.current = setTimeout(() => {
             startDragSequence(event);
         }, 250);
     }
  ...
  // Deteksi Folder saat Note digerakkan (Hover Effect)
  const handleDrag = (event: any, info: PanInfo) => {
      if (!setHoveredFolderId) return;

      // Cari elemen folder di bawah pointer
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     ...
     // Jika hanya geser sedikit (tap), anggap klik
     if (!isDragging && Math.abs(info.offset.x) < 5 && Math.abs(info.offset.y) < 5) {
         return; // Biarkan onClick handle ini
     }
     ...
     if (isDragging) {
         ...
         if (folderEl) {
             const targetId = folderEl.getAttribute('data-folder-id');
             if (targetId !== note.folderId) {
                 onMove(targetId);
                 return;
             }
         }
     } else {
         // SWIPE LOGIC (Hanya jika tidak sedang mode drag bebas)
  ...
  return (
    <motion.div 
       ...
       transition={springTransition}
       // Z-Index: 9999 saat drag, 50 saat peeking, 0 normal
    ...
    >
        <motion.div
           ref={cardRef}
           // DRAG CONFIGURATION
           drag={isDragging ? true : "x"} // Jika dragging: bebas, Jika tidak: cuma horizontal (swipe)
           dragControls={dragControls}
           dragListener={false} // PENTING: Kita trigger manual via startDragSequence
           dragConstraints={isDragging ? undefined : { left: 0, right: 0 }}
           dragElastic={isDragging ? 0 : 0.5}
           dragSnapToOrigin={true} // WAJIB: Kalau dilepas di tempat kosong, balik ke asal
           onDrag={handleDrag}
           onDragEnd={handleDragEnd}
           
           // onClick handler standar
           onClick={(e) => { 
               if (!isDragging) {
                   e.stopPropagation(); 
                   onClick(); 
           ...
           `}
           // FIX TOUCH ACTION: Mencegah scroll layar saat drag note
