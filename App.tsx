                ...
                <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-3">
                        {Object.keys(colors).map(c => {
                             ...
                             return <button key={c} onClick={() => onUpdate(folder.id, {color: c})} className={`w-10 h-10 rounded-full ${bgClass} ${isActive ? 'ring-4 ring-offset-2 ring-indigo-500 dark:ring-offset-zinc-900' : ''}`} />
                        })}
                    </div>
                </div>
                <div>
...
export default function App() {
  ...
  // --- REF MIRROR (SENJATA RAHASIA BIAR GAK KOSONG) ---
  ...
  // --- LOGIC MOVE (PAKE REF - ANTI GAGAL) ---
  const handleMoveSelected = (targetId: string | null) => {
      // Baca langsung dari Ref (Data terbaru)
      const idsToMove = Array.from(selectedIdsRef.current);
      ...
      setNotes(prevNotes => {
          return prevNotes.map(n => {
              if (idsToMove.includes(n.id)) {
                  // Paksa update timestamp biar React sadar
  ...
  return (
    <div className="pb-32 px-4 pt-6 md:px-8 max-w-7xl mx-auto min-h-screen" onClick={() => { setExpandedFolderId(null); if (isSelectionMode) handleExitSelection(); }}>
      <header className="mb-8 flex items-center gap-4" onClick={(e) => e.stopPropagation()}>
         ...
         <div className={`flex-1 relative rounded-full flex items-center px-4 py-3 transition-colors ${isDark ? 'bg-zinc-800' : 'bg-gray-300'}`}>
