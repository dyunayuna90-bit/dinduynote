...
import { 
  ...
  FilePlus, FolderPlus, Sparkles, Wand2, Languages, Eraser, Bot, Loader2, Copy, Eye, EyeOff, Save, Type
} from 'lucide-react';
...
// --- GEMINI AI HELPER (FIX: CLEAN OUTPUT) ---
...
const generateAIContent = async (prompt: string, apiKey: string) => {
    ...
    try {
        ...
        let text = data.candidates?.[0]?.content?.parts?.[0]?.text || "Maaf, saya tidak dapat menghasilkan respons saat ini.";
...
const ToolbarButton = ({ onClick, icon: Icon, active = false, isDark, appTheme, label }: any) => {
    ...
    return (
      <button 
        ...
        className={`p-3 rounded-xl transition-colors flex-shrink-0 flex items-center gap-2 ${active ? `${theme.bg} text-white` : isDark ? 'text-zinc-400 hover:text-white hover:bg-zinc-800' : 'text-zinc-600 hover:text-zinc-900 hover:bg-zinc-200'}`}
      >
        <Icon size={20} />
...
// --- Modals (FIX: Solid/Dimmed BG, No Blur) ---
...
const AIActionModal = ({ isOpen, onClose, noteContent, onApply, isDark, appTheme, apiKey }: any) => {
    ...
    const handleAction = async (action: string) => {
        ...
        // PROMPT TUNING FOR CLEANER OUTPUT
        switch (action) {
            case 'summarize': prompt = `Ringkaslah teks berikut ini menjadi paragraf singkat dalam bahasa Indonesia. Jangan gunakan bullet points, bold, atau format markdown lainnya. Gunakan teks biasa:\n\n${content}`; break;
            case 'continue': prompt = `Lanjutkan tulisan berikut ini secara logis dan kreatif (pertahankan gaya bahasa asli). Jangan gunakan format markdown:\n\n${content}`; break;
            case 'fix': prompt = `Perbaiki tata bahasa, ejaan, dan tanda baca teks berikut (pertahankan bahasa asli). Jangan ubah makna, hanya perbaiki struktur. Jangan gunakan markdown:\n\n${content}`; break;
            case 'translate': prompt = `Terjemahkan teks berikut ke Bahasa Inggris (jika Indonesia) atau Bahasa Indonesia (jika Inggris). Jangan gunakan markdown:\n\n${content}`; break;
            case 'idea': prompt = `Berikan 1 ide topik menarik untuk ditulis di catatan harian atau jurnal. Berikan dalam satu paragraf pendek tanpa markdown.`; break;
        }
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={springTransition} onClick={e => e.stopPropagation()} className={`w-full max-w-md p-6 rounded-[2rem] shadow-2xl max-h-[85vh] overflow-y-auto flex flex-col ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}>
                ...
                {result && (
                    <div className="flex flex-col flex-1 overflow-hidden">
                        <div className={`p-4 rounded-2xl mb-4 text-sm leading-relaxed overflow-y-auto max-h-[40vh] whitespace-pre-wrap ${isDark ? 'bg-zinc-800' : 'bg-zinc-100'}`}>
                            {result}
                        </div>
...
// --- MAIN SETTINGS MODAL (FIX: Save Button for API Key) ---
const SettingsModal = ({ isOpen, onClose, isDark, toggleTheme, onExport, onImport, appTheme, setAppTheme, apiKey, setApiKey }: any) => {
  ...
  useEffect(() => { if (isOpen) setTempKey(apiKey); setIsSaved(false); }, [isOpen, apiKey]);
            ...
            <div className={`p-4 rounded-2xl ${isDark ? 'bg-zinc-800' : 'bg-zinc-100'}`}>
             ...
             <div className={`p-4 rounded-2xl ${isDark ? 'bg-zinc-800' : 'bg-zinc-100'}`}>
                <div className="flex items-center justify-between mb-2">
                    ...
                    {isSaved && <span className="text-xs text-green-500 font-bold flex items-center gap-1"><Check size={12}/> Saved</span>}
                </div>
                <div className="flex gap-2">
                    <div className="relative flex-1">
                        ...
                        />
                        <button onClick={() => setShowKey(!showKey)} className="absolute right-3 top-3 opacity-50 hover:opacity-100">{showKey ? <EyeOff size={16}/> : <Eye size={16}/>}</button>
                    </div>
                    <button onClick={handleSaveKey} className={`p-3 rounded-xl ${theme.bg} text-white hover:opacity-90 transition-opacity`}><Save size={20}/></button>
                </div>
                <p className="text-[10px] mt-2 opacity-60 leading-tight">Key disimpan lokal di perangkat ini.</p>
            </div>
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, isDark, showSettings, onCloseSettings, onOpenSettings, appTheme, apiKey }: any) => {
  ...
  const checkFormats = () => {
    ...
    if (document.queryCommandState('justifyFull')) formats.push('justifyFull'); // Check for justify full
