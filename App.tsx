...
// --- Note Editor (Dark Mode Force Black) ---
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark, showSettings, onCloseSettings, onOpenSettings }: any) => {
  ...
  // FIX: Force Black Background in Dark Mode
...
export default function App() {
  ...
  const executeDelete = (type: string, id: string | null) => {
      ...
      if (type === 'note') { setNotes(prev => prev.filter(n => n.id !== id)); if (activeNoteId === id) setActiveNoteId(null); } 
          ...
          // FIX: Move notes to root when deleting folder
  ...
  // --- Logic Eject (Keluarkan Catatan dari Folder) ---
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          ...
          <Dashboard 
            ...
            onEjectNote={ejectNote} // Pass Eject Function
             ...
             <>
               {folders.map((folder: Folder) => (
                  <FolderItem 
                    ...
                    onDeleteNote={onEjectNote} // Pass Eject Logic Here
...
// --- Folder Item (Fixed: Swipe Eject & Layout Bug) ---
  ...
  return (
    ...
    >
        <div className={`relative w-full rounded-3xl ${isExpanded ? '' : 'h-full'}`}>
            <motion.div
                // FIX: Enable Drag even if Expanded (Safe Delete)
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-6 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-4">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    onDelete={() => onDeleteNote(note.id)} // This triggers Eject
                                    isPeeking={peekingNoteId === note.id}
                                    onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)}
                                />
                            ))}
    ...
    >
        <div className="relative w-full h-full">
            <motion.div
               ...
               `}
            >
               <div className="p-5 h-full flex flex-col relative"> {/* Reduced padding to p-5 */}
                   {safeIsPeeking ? (
                       ...
                       <>
                          <h3 className="text-xl font-bold leading-tight line-clamp-2 relative z-10">{note.title || "Untitled"}</h3> {/* Line Clamp 2 */}
