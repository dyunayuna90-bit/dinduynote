...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark, showSettings, onCloseSettings, onOpenSettings }: any) => {
  ...
  return (
    // FIX: Gunakan h-[100dvh] untuk handle mobile keyboard resize dan flex-col
    <div className={`fixed inset-0 z-[100] flex flex-col h-[100dvh] ${themeClass}`}>
       ...
       {/* Editor Area - Flex Grow (Isi sisa ruang) */}
       ...
       <div className={`flex-none w-full p-2 sticky bottom-0 ${isDark ? 'bg-zinc-900' : 'bg-white'} shadow-[0_-4px_20px_rgba(0,0,0,0.1)] z-50`}>
...
export default function App() {
  ...
  // NEW: State untuk drag dari folder yang di-close
  ...
  const executeDelete = (type: string, id: string | null) => {
      ...
      if (type === 'note') { setNotes(prev => prev.filter(n => n.id !== id)); if (activeNoteId === id) setActiveNoteId(null); } 
          ...
          // Logic baru: Delete folder = Notes tumpah ke luar (Dashboard)
  ...
  // Logic khusus: Drag dari folder -> Tutup folder -> Render di root sementara
  const handleDragStartFromFolder = (noteId: string) => {
      setDraggingNoteId(noteId);
      setExpandedFolderId(null);
  };

  // Logic khusus: Drag end (dari mana saja)
  const handleDragEndGlobal = (noteId: string, targetFolderId: string | null) => {
      setDraggingNoteId(null);
      // Jika targetFolderId null, berarti user drop di "Dashboard" atau tidak di folder manapun.
      // Behavior: Kalau drop di folder, pindah. Kalau drop di kosong, pindah ke root?
      // User request: "Jika posisinya belum ngedrop dimanapun, otomatis catatan balik lagi ke folder sebelumnya."
      // TAPI jika drop di "Dashboard" (targetFolderId === null), apakah itu "dimanapun"?
      // Kita asumsikan "move" event hanya trigger kalau user beneran drop di atas target folder.
      // Kalau user cuma lepas jari di ruang kosong, `targetFolderId` dari NoteCard mungkin null.
      // NoteCard akan memanggil onMove dengan `null` hanya jika didrop di area kosong dashboard.
      // Tapi kita butuh logic "Revert".
      
      // Di `NoteCard`, onMove dipanggil dengan `foundFolderId`. Jika tidak nemu folder, dia kirim `null`.
      // Jika `null` dikirim, dan asalnya dari folder, kita HARUS balikin ke folder asal (jangan ubah state).
      // KECUALI user emang mau move ke Dashboard?
      // Untuk amannya, kita ikuti user: "Balik ke folder sebelumnya".
      // Jadi: Kita HANYA ubah `folderId` jika `targetFolderId` TIDAK null.
      // Kalo mau move ke Dashboard, user bisa pake fitur Selection.
      
      if (targetFolderId !== null) {
          handleSingleNoteMove(noteId, targetFolderId);
      } else {
          // Do nothing (data tetap di folder lama), visual snapback diurus Framer Motion
      }
  };
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          ...
          <Dashboard 
            ...
            // Update logic delete folder:
            onDeleteFolder={(id) => executeDelete('folder', id)} 
...
const Dashboard = ({ 
  ...
  // Logic Filter: Root Notes + Dragging Note (meski dia punya folderId, dia harus render di luar saat di-drag)
  const rootNotes = notes.filter((n: Note) => n.folderId === null || n.id === draggingNoteId);
  const colors = isDark ? DARK_COLORS : LIGHT_COLORS;
           ...
           {searchQuery ? filteredNotes.map((note: Note) => (
             ...
             <>
               {folders.map((folder: Folder) => (
                  <FolderItem 
                    key={folder.id} folder={folder} notes={notes.filter((n: Note) => n.folderId === folder.id && n.id !== draggingNoteId)}
               ...
               {rootNotes.map((note: Note) => (
                  <NoteCard 
                    ...
                    // Logic Move dimodifikasi untuk handle drag out
                    onMove={(targetFolderId: string | null) => {
                         if (draggingNoteId === note.id) {
                             onDragEndGlobal(note.id, targetFolderId);
                         } else {
                             onMoveSingleNote(note.id, targetFolderId);
                         }
                    }}
                    setHoveredFolderId={setHoveredFolderId}
                    inFolder={false}
                    ...
                    // Force drag mode if this is the draggingNote
...
// --- Folder Item (FIX 3: SWIPE TO DELETE) ---
...
const FolderItem = ({ 
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     if (Math.abs(info.offset.x) > 100 && !isSelectionMode && !isExpanded) {
         onDelete(); // Delete folder -> spill notes
     }
  };
  ...
  return (
    ...
    >
        <div className={`relative w-full rounded-3xl ${isExpanded ? '' : 'h-full'}`}>
            <motion.div
                // Enable Drag only if NOT expanded and NOT in selection mode
...
// --- Note Card (FIX 2 & 4: STUCK BUG & FOLDER DRAG) ---

const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove, setHoveredFolderId, searchQuery, onDragStartFromFolder, forceDragMode }: any) => {
  ...
  // Jika forceDragMode aktif (dari Folder Drag), otomatis start dragging
  useEffect(() => {
     if (forceDragMode) {
         setIsDragging(true);
         // Kita butuh sedikit delay atau akses ke event asli, tapi karena struktur React,
         // kita biarkan user menekan lagi atau (better) visual sudah floating,
         // user tinggal gerakin.
         // NOTE: Secara teknis Framer Motion butuh pointer down event untuk start drag.
         // Jadi user harus menahan jari. Flow:
         // 1. User tahan di folder -> Timer jalan -> `onDragStartFromFolder` dipanggil.
         // 2. Folder tutup -> `NoteCard` ini unmount di folder, remount di Dashboard dengan `forceDragMode`.
         // 3. User MASIH menahan jari. Browser event pointer masih aktif.
         // 4. Kita harus nyambungin event itu. Ini tricky di React.
         // WORKAROUND: User akan melihat note "melayang" sesaat, lalu bisa lanjut drag.
     }
  }, [forceDragMode]);
  ...
  const startDragSequence = (event: React.PointerEvent) => {
      ...
      // KHUSUS FOLDER:
      if (inFolder) {
          if (onDragStartFromFolder) {
              onDragStartFromFolder(); // Trigger App-level state change
              // Jangan setIsDragging(true) disini karena komponen bakal unmount
          }
          return;
      }
  ...
  const handlePointerMove = (event: React.PointerEvent) => {
      pointerCurrentPosition.current = { y: event.clientY };
      if (longPressTimer.current && pointerStartPosition.current) {
  ...
  // FIX STUCK BUG: Tambahkan reset di PointerUp
  const handlePointerUp = () => {
     ...
     // Fail-safe: Jika isDragging true tapi event dragEnd ga jalan sempurna
     // atau user cuma tap-release cepet pas transisi mode
     if (isDragging) {
         // Tunggu bentar biar dragEnd asli jalan dulu
         setTimeout(() => {
             setIsDragging(false);
         }, 100);
     }
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     ...
     onMove(foundFolderId); // Kirim null jika drop di space kosong
  };
