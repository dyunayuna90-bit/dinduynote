...
// [FIX] Auto Scroll yang lebih stabil tanpa mengganggu drag logic
const useAutoScroll = (isDragging: boolean) => {
  useEffect(() => {
    ...
    const scrollLoop = () => {
      // Kita perlu posisi mouse global, tapi karena keterbatasan React event di hook,
      // kita biarkan user scroll manual via gesture drag ke tepi layar (browser default behavior)
      // atau implementasi custom scroll container ref di masa depan.
      // Untuk stabilitas drag & drop, kode auto-scroll kompleks dihapus sementara
      // karena sering menyebabkan "stuck" pada koordinat translate.
    };
             ...
             <>
               ...
               {rootNotes.map((note: Note) => (
                  <NoteCard 
                    ...
                    onMove={(targetFolderId: string | null) => onMoveSingleNote(note.id, targetFolderId)}
                    setHoveredFolderId={setHoveredFolderId}
                    // Fix Bug 3: Jika di root, inFolder=false
                    inFolder={false}
                  />
               ))}
             </>
...
const FolderItem = ({ 
  ...
  return (
    <motion.div 
       ...
       data-folder-id={folder.id} // Marker untuk deteksi collision
    >
        <div className={`relative w-full rounded-3xl ${isExpanded ? '' : 'h-full'}`}>
            <motion.div
                // [FIX 5] Visual Feedback: Scale up saat ada note melayang di atasnya
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-6 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-4">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    // [FIX 3] Swipe to Eject logic (not dragging)
...
// --- Note Card (FIX LENGKAP) ---

const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove, setHoveredFolderId }: any) => {
  ...
  // [FIX 4] Logika baru untuk handle press
  const handlePointerDown = (e: React.PointerEvent) => {
     if (isSelectionMode) return;
     
     // [FIX 3] Cegah Drag jika di dalam folder (kecuali swipe yg dihandle gesture terpisah)
     if (inFolder) return;

     const startDrag = () => {
        setIsDragMoveMode(true);
        if (navigator.vibrate) navigator.vibrate(50);
        controls.start({ scale: 1.05, rotate: 2, zIndex: 9999 }); // [FIX 6] High z-index
     };

     if (safeIsPeeking) {
         // [FIX 4] Peeking -> Shrink -> Drag
         // Close peek first immediately
         onPeek(false);
         // Wait a tiny bit for render update, then start drag
         setTimeout(startDrag, 100);
     } else {
         // Standard long press
         longPressTimer.current = setTimeout(startDrag, 250); 
     }
  };
  ...
  const handleDrag = (event: any, info: PanInfo) => {
      if (isDragMoveMode && setHoveredFolderId) {
          // [FIX 1 & 5] Manual Bounding Box Collision Detection
          // Kita cari folder elemen secara manual karena elementsFromPoint sering gagal ketutup note sendiri
          const folders = document.querySelectorAll('[data-folder-id]');
          let foundFolderId: string | null = null;

          const noteRect = cardRef.current?.getBoundingClientRect();
          if (noteRect) {
              ...
              folders.forEach((el) => {
                  ...
                  if (
                      noteCenter.x >= rect.left &&
                      noteCenter.x <= rect.right &&
                      noteCenter.y >= rect.top &&
                      noteCenter.y <= rect.bottom
                  ) {
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     ...
     if (isDragMoveMode) {
         // [FIX 2] Logic Drop vs Snap Back
         // Cek apakah mouse berada di atas folder valid (via hovered state terakhir)
         
         // Re-check collision one last time to be sure
         const folders = document.querySelectorAll('[data-folder-id]');
         ...
         if (noteRect) {
            ...
            folders.forEach((el) => {
                const rect = el.getBoundingClientRect();
                if (noteCenter.x >= rect.left && noteCenter.x <= rect.right && noteCenter.y >= rect.top && noteCenter.y <= rect.bottom) {
                    targetFolderId = el.getAttribute('data-folder-id');
                }
            });
         }
         ...
         if (targetFolderId && targetFolderId !== note.folderId) {
             ...
             controls.start({ scale: 0, opacity: 0 }).then(() => {
                 controls.set({ scale: 1, opacity: 1, x: 0, y: 0, rotate: 0, zIndex: 10 });
             });
         } else {
             // [FIX 2 & 5] Snap Back if no target
             controls.start({ x: 0, y: 0, rotate: 0, scale: 1, zIndex: 10, transition: springTransition });
         }
     } else {
         // Swipe Action (Only allowed inside folder or specific view)
  ...
  return (
    <motion.div 
       layout 
       transition={springTransition}
       className={`${safeIsPeeking ? 'col-span-2 row-span-2 z-[50]' : 'col-span-1 aspect-square'} relative`} // [FIX 6] z-50 for peeking, drag will override
       onPointerDown={handlePointerDown}
       onPointerUp={handlePointerUp}
       onPointerLeave={handlePointerUp}
       onClick={(e) => { e.stopPropagation(); if (!isDragMoveMode) onClick(); }}
    ...
    >
        <div className="relative w-full h-full">
            <motion.div
               ...
               dragConstraints={isDragMoveMode ? { top: 0, left: 0, right: 0, bottom: 0 } : { left: 0, right: 0 }} // [FIX 2] Free drag
               dragElastic={isDragMoveMode ? 0 : 0.5} // No elastic on drag mode
               dragMomentum={false} // [FIX 2] No throwing momentum
               onDrag={handleDrag}
               ...
               `}
               // [FIX 6] Z-Index dinamis
               style={{ zIndex: isDragMoveMode ? 9999 : (safeIsPeeking ? 50 : 10) }}
            >
               ...
               {/* Controls for Peeking */}
               {!isSelectionMode && onPeek && !isDragMoveMode && (
                   <button 
                     onPointerDown={(e) => e.stopPropagation()} // Prevent drag trigger on button
