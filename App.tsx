...
export const APP_THEMES: Record<AppThemeColor, { bg: string, text: string, ring: string, hover: string, border: string, lightBg: string, tabActive: string }> = {
    default: { bg: 'bg-zinc-900', text: 'text-zinc-900', ring: 'ring-zinc-900', hover: 'hover:bg-zinc-100', border: 'border-zinc-200', lightBg: 'bg-[#f8f9fa]', tabActive: '!bg-zinc-900 !text-white' },
    violet: { bg: 'bg-violet-600', text: 'text-violet-600', ring: 'ring-violet-600', hover: 'hover:bg-violet-50', border: 'border-violet-200', lightBg: 'bg-violet-100', tabActive: '!bg-violet-600 !text-white' },
    blue:   { bg: 'bg-blue-600', text: 'text-blue-600', ring: 'ring-blue-600', hover: 'hover:bg-blue-50', border: 'border-blue-200', lightBg: 'bg-blue-100', tabActive: '!bg-blue-600 !text-white' },
    green:  { bg: 'bg-emerald-600', text: 'text-emerald-600', ring: 'ring-emerald-600', hover: 'hover:bg-emerald-50', border: 'border-emerald-200', lightBg: 'bg-emerald-100', tabActive: '!bg-emerald-600 !text-white' },
    orange: { bg: 'bg-orange-600', text: 'text-orange-600', ring: 'ring-orange-600', hover: 'hover:bg-orange-50', border: 'border-orange-200', lightBg: 'bg-orange-100', tabActive: '!bg-orange-600 !text-white' },
    rose:   { bg: 'bg-rose-600', text: 'text-rose-600', ring: 'ring-rose-600', hover: 'hover:bg-rose-50', border: 'border-rose-200', lightBg: 'bg-rose-100', tabActive: '!bg-rose-600 !text-white' },
    yellow: { bg: 'bg-yellow-500', text: 'text-yellow-600', ring: 'ring-yellow-500', hover: 'hover:bg-yellow-50', border: 'border-yellow-200', lightBg: 'bg-yellow-100', tabActive: '!bg-yellow-500 !text-black' },
    teal:   { bg: 'bg-teal-600', text: 'text-teal-600', ring: 'ring-teal-600', hover: 'hover:bg-teal-50', border: 'border-teal-200', lightBg: 'bg-teal-100', tabActive: '!bg-teal-600 !text-white' },
    indigo: { bg: 'bg-indigo-600', text: 'text-indigo-600', ring: 'ring-indigo-600', hover: 'hover:bg-indigo-50', border: 'border-indigo-200', lightBg: 'bg-indigo-100', tabActive: '!bg-indigo-600 !text-white' },
    lime:   { bg: 'bg-lime-600', text: 'text-lime-600', ring: 'ring-lime-600', hover: 'hover:bg-lime-50', border: 'border-lime-200', lightBg: 'bg-lime-100', tabActive: '!bg-lime-600 !text-black' },
    fuchsia:{ bg: 'bg-fuchsia-600', text: 'text-fuchsia-600', ring: 'ring-fuchsia-600', hover: 'hover:bg-fuchsia-50', border: 'border-fuchsia-200', lightBg: 'bg-fuchsia-100', tabActive: '!bg-fuchsia-600 !text-white' },
};
...
const generateAIContent = async (prompt: string, apiKey: string) => {
    ...
    } catch (error) { return "Terjadi kesalahan koneksi. Silakan coba lagi."; }
};
...
const AIActionModal = ({ isOpen, onClose, noteContent, onApply, isDark, appTheme, apiKey }: any) => {
    ...
    const handleAction = async (action: string) => {
        ...
        switch (action) {
            case 'summarize': prompt = `Ringkaslah teks ini (Indo). Tanpa pengantar, tanpa markdown/bold:\n\n${content}`; break;
            case 'continue': prompt = `Lanjutkan tulisan ini secara natural. Tanpa pengantar, tanpa markdown:\n\n${content}`; break;
            case 'fix': prompt = `Perbaiki tata bahasa teks ini. Tanpa pengantar, tanpa markdown:\n\n${content}`; break;
            case 'translate': prompt = `Terjemahkan teks ini ke Indo/Inggris. Tanpa pengantar, tanpa markdown, langsung hasil:\n\n${content}`; break;
            case 'idea': prompt = `Berikan 1 ide topik jurnal pendek. Tanpa pengantar, tanpa markdown.`; break;
        }
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, isDark, showSettings, onCloseSettings, onOpenSettings, appTheme, apiKey }: any) => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
          ...
          if (file.size > 8 * 1024 * 1024) {
              alert("Image size exceeds 8MB limit. Please upload a smaller image.");
              return;
          }
  ...
  const handleAIApply = (text: string, mode: 'replace' | 'append') => {
      ...
      // Safe replace logic: keep images if replacing
      if (mode === 'replace') {
          // 1. Ambil semua gambar dari konten lama
          const tempDiv = document.createElement('div');
          tempDiv.innerHTML = note.content;
          const images = tempDiv.getElementsByTagName('img');
          const savedImages = Array.from(images).map(img => img.outerHTML).join('<br/>');

          // 2. Gabungin hasil AI + Gambar yang diselamatin
          const newContent = text + (savedImages ? `<br/><br/>${savedImages}` : '');
