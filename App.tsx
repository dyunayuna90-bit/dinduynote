...
// [FIX 4] Visual Viewport Hook untuk Keyboard
const useVisualViewport = () => {
  ...
  useEffect(() => {
    ...
    const handler = () => {
      // visualViewport.height adalah tinggi area yang BENAR-BENAR terlihat (di atas keyboard)
    ...
    window.addEventListener('resize', handler); // Fallback
...
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark, showSettings, onCloseSettings, onOpenSettings }: any) => {
  ...
  // [FIX 4] Gunakan tinggi visual viewport
  ...
  return (
    // FIX 4: Gunakan height dinamis dari viewportHeight
    <div 
        className={`fixed top-0 left-0 right-0 z-[100] flex flex-col ${themeClass}`}
...
export default function App() {
  ...
  // [FIX 2] Global state untuk melacak folder asal drag
  const [draggingFromFolderId, setDraggingFromFolderId] = useState<string | null>(null);
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          ...
          <Dashboard 
            ...
            draggingFromFolderId={draggingFromFolderId} setDraggingFromFolderId={setDraggingFromFolderId}
          />
        )}
      </AnimatePresence>
...
const Dashboard = ({ 
  ...
  onMoveClick, hoveredFolderId, setHoveredFolderId, draggingFromFolderId, setDraggingFromFolderId
}: any) => {
        ...
        <motion.div layout transition={springTransition} className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
           {searchQuery ? filteredNotes.map((note: Note) => (
               <NoteCard 
                  ...
                  setDraggingFromFolderId={setDraggingFromFolderId}
               />
           )) : (
             <>
               {folders.map((folder: Folder) => (
                  <FolderItem 
                    ...
                    // [FIX 2] Props z-index
                    isSourceOfDrag={draggingFromFolderId === folder.id}
                    setHoveredFolderId={setHoveredFolderId}
                    setDraggingFromFolderId={setDraggingFromFolderId}
                  />
               ))}
...
const FolderItem = ({ 
  ...
  peekingNoteId, setPeekingNoteId, isHovered, isSourceOfDrag, setHoveredFolderId, setDraggingFromFolderId
}: any) => {
  ...
  return (
    <motion.div 
       ...
       // [FIX 2] Z-index Pop: Jika folder ini adalah sumber drag, angkat z-index tinggi sekali
       style={{ zIndex: isSourceOfDrag ? 9999 : (isHovered ? 50 : 1) }}
    >
        <div className={`relative w-full rounded-3xl ${isExpanded ? '' : 'h-full'}`}>
            <motion.div
                ...
                // [FIX 1] Feedback visual saat hover note masuk
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-6 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-4">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    // [FIX 1] Teruskan setHoveredFolderId ke NoteCard dalam folder
                                    setHoveredFolderId={setHoveredFolderId} 
                                    setDraggingFromFolderId={(val: any) => {
                                        // Saat note mulai di drag, set ID folder ini sebagai source
                                        if(val) setDraggingFromFolderId(folder.id);
                                        else setDraggingFromFolderId(null);
                                    }}
                                />
                            ))}
...
const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove, setHoveredFolderId, searchQuery, setDraggingFromFolderId }: any) => {
  ...
  const startDragSequence = (event: React.PointerEvent) => {
      ...
      // [FIX 2] Beritahu App bahwa note ini berasal dari folder ini (jika inFolder)
      if (inFolder && setDraggingFromFolderId) {
          setDraggingFromFolderId(true); 
      }
  ...
  const handlePointerDown = (event: React.PointerEvent) => {
     ...
     // [FIX 3] Mempercepat timer sedikit agar "balapan" dengan scroll browser lebih cepat menang
     const delay = safeIsPeeking ? 150 : 250;

     longPressTimer.current = setTimeout(() => {
         if (safeIsPeeking) onPeek(false);
  ...
  const handlePointerMove = (event: React.PointerEvent) => {
      ...
      if (pointerStartPosition.current) {
          ...
          if (dist > 5) hasMoved.current = true;
  ...
  const handlePointerUp = () => {
     ...
     if (isDragging) {
         // Reset state global drag
         if (inFolder && setDraggingFromFolderId) {
             setDraggingFromFolderId(false);
         }
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     ...
     setIsDragging(false);
     if (setHoveredFolderId) setHoveredFolderId(null);
     if (inFolder && setDraggingFromFolderId) setDraggingFromFolderId(false);
