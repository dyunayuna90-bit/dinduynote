...
// MODIFIKASI: Animasi dipercepat (High Stiffness) namun tetap smooth (Damping disesuaikan)
...
const FolderItem = ({ 
  ...
  return (
    <motion.div 
       ...
       // MODIFIKASI: Menambahkan data-folder-id agar bisa dideteksi saat drop
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-6 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-4">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    isPeeking={peekingNoteId === note.id}
                                    onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)}
                                    // MODIFIKASI: Eject dari folder (move to null) saat di drag keluar
...
// --- Note Card (MODIFIKASI: Long Press Drag & Drop + Peeking Swipe + Regex Fix) ---
...
const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove }: any) => {
  ...
  const handlePointerDown = () => {
     ...
     longPressTimer.current = setTimeout(() => {
         ...
         // Haptic feedback (vibration) jika didukung
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     // Jika mode Drag Move aktif (Drag to Folder)
     if (isDragMoveMode) {
         ...
         } else if (inFolder && Math.abs(info.offset.y) > 100) {
             // Opsional: Drag jauh keluar folder untuk eject ke dashboard? 
             // Saat ini biarkan saja kembali
         }
     } 
     // Jika mode swipe biasa (Delete)
     else {
  ...
  // MODIFIKASI: Regex Fix untuk &nbsp; dan tag HTML lainnya
  const previewText = note.content
      .replace(/&nbsp;/g, ' ') // Hapus &nbsp; spesifik
  ...
  return (
    ...
    >
        <div className="relative w-full h-full">
            <motion.div
               animate={controls}
               // MODIFIKASI: Logic Drag
               // 1. isDragMoveMode = true -> Drag bebas (ke segala arah)
               // 2. safeIsPeeking = true -> Drag X only (Swipe delete saat ngintip)
               // 3. Normal -> Drag X only (Swipe delete)
