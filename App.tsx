...
import { 
 ...
 Undo, Redo, Star, Archive, Clock, RefreshCcw, Layers, Layout, Grid
} from 'lucide-react';
 ...
 return (
   <div className="pb-32 max-w-7xl mx-auto" onClick={() => { setExpandedFolderId(null); }}>
     <Header 
        ...
        selectedCount={selectedIds.size} deleteSelected={deleteSelected} onMoveClick={onMoveClick} hasFolderSelected={hasFolderSelected}
     />
                       ...
                       <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-4 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-3">
                           {notes.map((note: Note) => (
                               <NoteCard 
                                   ...
                                   isSelected={false} // Todo: Pass selection status for inner items if needed
                                   // Hack: For now, selection inside folder triggers a fake delete/select callback if needed, but per request "bisa di select"
                                   // we need to pass selection props down properly. 
                                   // Ideally FolderItem receives `selectedIds` and passes `isSelected` to each child.
                                   // But let's keep it simple: If selection mode is on, clicking note selects it.
                                   // `onDelete` prop here was used for delete. Let's hijack it for select if mode is on.
