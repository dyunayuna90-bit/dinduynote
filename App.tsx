...
export default function App() {
  ...
  // --- LOGIC MOVE SELECTED (FIXED TOTAL: FORCE UPDATE) ---
  ...
  return (
    <div className="pb-32 px-4 pt-6 md:px-8 max-w-7xl mx-auto min-h-screen" onClick={() => { setExpandedFolderId(null); if (isSelectionMode) handleExitSelection(); }}>
      <header className="mb-8 flex items-center gap-4" onClick={(e) => e.stopPropagation()}>
         <button onClick={toggleSidebar} className={`p-3 rounded-full transition-colors ${isDark ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-slate-100 hover:bg-slate-200'}`}>
            <Menu size={24} />
         </button>
         {/* FIX: Search Bar Darker BG (Gray-200) */}
         <div className={`flex-1 relative rounded-full flex items-center px-4 py-3 transition-colors ${isDark ? 'bg-zinc-800' : 'bg-slate-200'}`}>
         ...
         <button onClick={() => isSelectionMode ? handleExitSelection() : setIsSelectionMode(true)} className={`p-3 rounded-full transition-colors ${isSelectionMode ? 'bg-indigo-600 text-white' : isDark ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-slate-100 hover:bg-slate-200'}`}>
            <CheckSquare size={24} />
         </button>
      </header>
        ...
        <motion.div layout transition={springTransition} className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
           {searchQuery ? filteredNotes.map((note: Note) => (
               <NoteCard 
                  ...
                  // FIX: Logic Selection di Search Mode
