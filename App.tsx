            ...
            >
               ...
               <div className="flex-1 overflow-y-auto space-y-2 mb-4 pr-1">
                  <button 
                    ...
                    className={`w-full p-4 rounded-2xl text-left font-medium flex items-center gap-3 border-2 transition-all ${
                        selectedTarget === 'root' 
...
export default function App() {
  ...
  // --- LOGIC MOVE SELECTED (DIPERBAIKI) ---
  const handleMoveSelected = (targetId: string | null) => {
      const idsToMove = Array.from(selectedIds);
      if (idsToMove.length === 0) return;
      setNotes(prevNotes => {
          return prevNotes.map(n => {
              if (idsToMove.includes(n.id)) { return { ...n, folderId: targetId, updatedAt: Date.now() }; }
              return n;
          });
      });
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-[#121212] text-[#f0f0f0]' : 'bg-[#fdfdfd] text-[#1a1c1e]'}`}>
      ...
      {/* MOVE DIALOG DIANGKAT KE APP UTAMA */}
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          ...
          <Dashboard 
            ...
            setShowMoveDialog={setShowMoveDialog} // Pass Trigger Only
...
const Dashboard = ({ 
  ...
  setShowMoveDialog // Receive trigger only
}: any) => {
