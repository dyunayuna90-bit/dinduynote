...
// MASS 0.5 = RINGAN BANGET (KURANGIN BEBAN RENDER)
// STIFFNESS 800 = TARIKAN KUAT (CEPAT)
// DAMPING 50 = REM PAKEM (GAK MEMBAL)
const springTransition = { type: "spring", stiffness: 800, damping: 50, mass: 0.5 }; 
const cardTransition = { type: "spring", stiffness: 900, damping: 50, mass: 0.6 }; 
...
// --- ULTIMATE QUICK CARD (NAVIGATOR EDITION v2) ---
const QuickCreateCard = ({ isOpen, setIsOpen, onCreateNote, onCreateFolder, onFullEditor, appTheme, isDark, onTabChange }: any) => {
    ...
    return (
        <motion.div
            ...
            // SHADOW LEBIH TIPIS & ELEGAN
            className={`fixed bottom-0 left-0 right-0 z-[80] rounded-t-[2rem] shadow-[0_-4px_20px_rgba(0,0,0,0.08)] border-t border-black/5 flex flex-col overflow-hidden ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-100'}`}
        >
            {/* --- NAVIGATOR DOCK --- */}
            <motion.div 
                ...
                dragElastic={0.02} // Stiff drag vertical
                onDragEnd={(e, info) => {
                    // Cek dominasi gerakan (X vs Y)
                    const xMove = Math.abs(info.offset.x);
                    const yMove = Math.abs(info.offset.y);

                    if (xMove > yMove && xMove > 50) {
                        // Dominan Horizontal -> Ganti Tab
                        if (info.offset.x < 0) onTabChange(1); // Swipe Kiri -> Next Tab
                        else onTabChange(-1); // Swipe Kanan -> Prev Tab
                    } else {
                        // Dominan Vertikal -> Buka/Tutup Card
                    ...
                    {!isOpen ? (
                        ...
                        >
                            ...
                            <div className="flex gap-3 flex-1">
                                <button 
                                    onPointerDown={(e) => e.stopPropagation()} // Biar gak drag card pas neken tombol
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between">
                                <span className="font-bold text-xl opacity-60">Quick Note</span>
