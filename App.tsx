...
// --- NEW MOVE DIALOG (WITH CONFIRM BUTTON) ---
    ...
    return (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <motion.div 
               initial={{ scale: 0.9, opacity: 0 }} 
               animate={{ scale: 1, opacity: 1 }} 
               className={`w-full max-w-sm rounded-3xl p-6 shadow-2xl flex flex-col max-h-[80vh] ${isDark ? 'bg-zinc-900 text-white' : 'bg-white text-black'}`} 
               onClick={e => e.stopPropagation()}
            >
...
export default function App() {
  ...
  // --- LOGIC MOVE SELECTED (FIXED: WITH NEW DIALOG) ---
      ...
      setNotes(prevNotes => {
          return prevNotes.map(n => {
              if (idsToMove.includes(n.id)) {
                  // Tambahkan updatedAt untuk memaksa React re-render
...
const Dashboard = ({ 
  ...
  setShowMoveDialog // Only need to open dialog
}: any) => {
