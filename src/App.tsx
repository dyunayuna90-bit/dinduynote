...
// --- CONSTANTS ---
// SUPER SATSET ANIMATION: High stiffness = instant reaction, High damping = no bounce
const springTransition = { type: "spring", stiffness: 700, damping: 40, mass: 1 }; 
const cardTransition = { type: "spring", stiffness: 600, damping: 35, mass: 0.8 }; // Khusus card biar smooth tapi ngebut
...
// --- ULTIMATE QUICK CARD (NAVIGATOR EDITION) ---
const QuickCreateCard = ({ isOpen, setIsOpen, onCreateNote, onCreateFolder, onFullEditor, appTheme, isDark, onTabChange }: any) => {
    ...
    return (
        <motion.div
            ...
            className={`fixed bottom-0 left-0 right-0 z-[80] rounded-t-[2rem] shadow-[0_-15px_50px_rgba(0,0,0,0.2)] border-t border-black/5 flex flex-col overflow-hidden ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-100'}`}
        >
            {/* --- NAVIGATOR DOCK --- */}
            <motion.div 
                ...
                onPanEnd={(e, info) => {
                    // Horizontal Swipe Logic (Switch Tabs)
            ...
            >
                <AnimatePresence mode="wait">
                    {!isOpen ? (
                        ...
                        >
                            {/* HANDLE JUGA DI SINI BIAR KONSISTEN */}
...
export default function App() {
  ...
  const [isQuickCardOpen, setIsQuickCardOpen] = useState(false); // MANAGED HERE FOR BACK BUTTON

  const theme = APP_THEMES[appTheme as AppThemeColor];
  
  const TABS_ORDER: TabType[] = ['all', 'favorites', 'folders', 'notes', 'trash'];

  // History Pop State Handling (Back Button Logic)
  ...
  useEffect(() => {
    ...
    const handlePopState = (event: PopStateEvent) => {
      ...
      // PRIORITY: Close Quick Card first if open
