...
// --- Note Editor (FIX KEYBOARD TOOLBAR) ---
// Menggunakan h-[100dvh] dan Sticky bottom untuk menjamin toolbar nempel keyboard
const NoteEditor = ({ note, onUpdate, onClose, onDelete, onMoveNote, folders, isDark, showSettings, onCloseSettings, onOpenSettings }: any) => {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex flex-col h-[100dvh] ${themeClass}`}>
       ...
       {/* Sticky Bottom ensures it sits on top of keyboard */}
       <div className={`flex-none w-full p-2 sticky bottom-0 z-50 ${isDark ? 'bg-zinc-900 border-t border-zinc-800' : 'bg-white border-t border-zinc-200'}`}>
...
export default function App() {
  ...
  // State khusus untuk drag dari dalam folder
  ...
  const executeDelete = (type: string, id: string | null) => {
      ...
      if (type === 'note') { setNotes(prev => prev.filter(n => n.id !== id)); if (activeNoteId === id) setActiveNoteId(null); } 
          ...
          // Hapus folder, tapi isi note nya DI KELUARKAN (jadi root)
  ...
  const handleSingleNoteMove = (noteId: string, targetFolderId: string | null) => {
      ...
      // Reset dragging child jika note sudah pindah
      ...
      <AnimatePresence mode="wait">
        {activeNoteId ? (
          ...
          <Dashboard 
            ...
            // Ubah pesan delete folder agar user tau note nya bakal keluar
            onDeleteFolder={(id) => requestDelete('folder', id, "Folder akan dihapus, isi catatan akan dikeluarkan ke Dashboard (Move Out).")} 
            ...
            draggingChildId={draggingChildId} setDraggingChildId={setDraggingChildId}
          />
        )}
      </AnimatePresence>
...
const Dashboard = ({ 
  ...
  draggingChildId, setDraggingChildId
}: any) => {
             ...
             <>
               {folders.map((folder: Folder) => (
                  <FolderItem 
                    ...
                    // Props baru untuk fitur drag keluar folder
                    draggingChildId={draggingChildId}
                    setDraggingChildId={setDraggingChildId}
                    setHoveredFolderId={setHoveredFolderId} // Pass ke anak
                  />
               ))}
...
// --- Folder Item (FIX FOLDER SWIPE & DRAG OUT) ---
...
const FolderItem = ({ 
  ...
  draggingChildId, setDraggingChildId, setHoveredFolderId
}: any) => {
  const themeClass = colors[folder.color] || colors.slate;

  // Handler Swipe Folder
  const handleDragEnd = (event: any, info: PanInfo) => {
      // Jika swipe horizontal > 100px, hapus folder (isi di-dump ke luar)
  ...
  return (
    <motion.div 
       ...
       style={{ zIndex: (isHovered || draggingChildId) ? 10 : 1 }}
    >
        <div className={`relative w-full rounded-3xl ${isExpanded ? '' : 'h-full'}`}>
            <motion.div
                // FITUR FOLDER SWIPE ENABLED
                ...
                // ANIMASI:
                // 1. Membesar jika ada note melayang di atasnya (isHovered)
                // 2. Mengecil jika ada anak note yang sedang di-drag keluar (draggingChildId) -> Efek "nguncup" visual
            ...
            >
                <div className={`p-5 flex flex-col ${isExpanded ? '' : 'h-full'}`}>
                    ...
                    {/* Sembunyikan count jika sedang expand */}
                ...
                <AnimatePresence>
                    {isExpanded && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-6 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-4">
                            {notes.map((note: Note) => {
                                // Logic "Hiding Sibling": Jika ada anak yang sedang di-drag, sembunyikan yang lain agar terlihat folder "kosong/nguncup"
                                const isBeingDragged = draggingChildId === note.id;
                                if (draggingChildId && !isBeingDragged) return null;

                                return (
                                  <NoteCard 
                                      key={note.id} note={note} isDark={isDark} colors={colors} inFolder={true}
                                      onClick={() => onOpenNote(note.id)} 
                                      onDelete={() => onMoveNote(note.id, null)} // Swipe to Eject
                                      isPeeking={peekingNoteId === note.id}
                                      onPeek={(val: boolean) => setPeekingNoteId(val ? note.id : null)}
                                      ...
                                      // Props Drag Out
                                      setHoveredFolderId={setHoveredFolderId}
                                      onDragStart={() => setDraggingChildId(note.id)}
                                      onDragEnd={() => setDraggingChildId(null)}
                                  />
                                );
                            })}
                            
                            {/* Tombol Add Note disembunyikan jika sedang drag */}
                            {!draggingChildId && (
...
// --- Note Card (FIX FORCE DROP & DRAG OUT) ---

const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove, setHoveredFolderId, searchQuery, onDragStart, onDragEnd: propsOnDragEnd }: any) => {
  ...
  // --- LOGIKA DRAG YANG BARU ---
  
  const startDragSequence = (event: React.PointerEvent) => {
      // PENTING: Jika search aktif, matikan fitur Drag sepenuhnya
      if (searchQuery) return;
      
      if (navigator.vibrate) navigator.vibrate(50);
      
      setIsDragging(true);
      if (onDragStart) onDragStart(); // Notify folder parent

      // "Curi" pointer dari swipe handler dan paksa mulai drag bebas
      dragControls.start(event, { snapToCursor: true });
  };

  const handlePointerDown = (event: React.PointerEvent) => {
     if (isSelectionMode) return;
     // Note: Sekarang inFolder BOLEH drag
  ...
  const handlePointerMove = (event: React.PointerEvent) => {
      ...
      // Jika user mulai swipe (gerak > 10px), batalkan niat Long Press
  ...
  const handlePointerUp = () => {
     ...
     // FIX BUG "STUCK" / "NGAMBANG":
     // Jika isDragging true tapi event pointerUp terjadi, kita harus memaksa drag selesai.
     // Framer Motion kadang tidak memanggil onDragEnd jika tidak ada gerakan signifikan.
     if (isDragging) {
        setIsDragging(false);
        if (propsOnDragEnd) propsOnDragEnd();
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     ...
     // Logic Drop:
     // 1. Jika drop di folder baru -> Pindah
     // 2. Jika drop di "kosong" saat sedang di folder -> Pindah ke Root (Eject) -> atau Snap Back? 
     //    Request user: "jika user drag and drop catatan tapi posisinya belum ngedrop dimanapun, otomatis catatan balik lagi ke folder sebelumnya"
     //    Jadi kita hanya pindah jika foundFolderId ada dan berbeda.
     
     if (foundFolderId && foundFolderId !== note.folderId) {
         onMove(foundFolderId);
     }
     
     // Jika dragging dari folder ke area kosong (bukan folder), Framer Motion akan snapToOrigin,
     // yang mana secara visual akan balik ke dalam folder yang "nguncup" tadi, lalu folder mekar lagi (karena setIsDragging false).
     // Ini sesuai request: "otomatis catatan masuk lagi ke folder sebelum nya"
  };
  ...
  return (
    ...
    >
        <motion.div
           ...
           dragSnapToOrigin={true} // Kunci untuk balik ke asal jika dilepas sembarangan
