...
// [FIX #2] Hook untuk Auto Scroll saat Dragging
const useAutoScroll = (isDragging: boolean) => {
  useEffect(() => {
    if (!isDragging) return;

    let animationFrameId: number;
    const scrollSpeed = 10; // Kecepatan scroll
    const threshold = 100; // Jarak pixel dari tepi untuk memicu scroll

    const checkScroll = () => {
      // Kita perlu membaca posisi pointer global terakhir, tapi di React hook agak tricky.
      // Kita akan handle logic scroll ini di event listener 'pointermove' global sementara
    };
    ...
    // Menggunakan interval untuk scroll terus menerus jika pointer diam di tepi
    const intervalId = setInterval(() => {
       // Catatan: implementasi scroll yang sangat halus butuh tracking koordinat mouse state ref
       // Di sini kita pakai pendekatan event listener sederhana di bawah.
    }, 16);
...
// --- Modals (Tidak Berubah Signifikan) ---
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-[#121212] text-[#f0f0f0]' : 'bg-[#fdfdfd] text-[#1a1c1e]'}`}>
      ...
      {!activeNoteId && !isSelectionMode && !searchQuery && (
        // [FIX #1] Z-Index ditinggikan menjadi z-[200] agar tombol plus tidak tertutup
...
const FolderItem = ({ 
  ...
  return (
    <motion.div 
       ...
       // [FIX #5] Atribut ini PENTING agar folder bisa dideteksi saat drag & drop, baik saat kuncup maupun mekar
    ...
    >
        <div className={`relative w-full rounded-3xl ${isExpanded ? '' : 'h-full'}`}>
            <motion.div
                ...
                // [FIX #5] Pastikan area drop mencakup seluruh folder yang sedang expand
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={springTransition} className="p-6 pt-0 grid grid-cols-2 lg:grid-cols-3 gap-4">
                            {notes.map((note: Note) => (
                                <NoteCard 
                                    ...
                                    // [FIX #4] Jika di swipe dalam folder, Eject (pindah ke null) alih-alih hapus permanen
...
// --- Note Card (MODIFIKASI: Long Press, Auto Scroll, Fix Stuck) ---

const NoteCard = ({ note, onClick, onDelete, inFolder, isDark, colors, isSelected, isSelectionMode, isPeeking, onPeek, onMove }: any) => {
  ...
  // Aktifkan Hook Auto Scroll
  useAutoScroll(isDragMoveMode);

  const handlePointerDown = (e: React.PointerEvent) => {
     if (isSelectionMode || safeIsPeeking) return;
     // [FIX #3] Waktu tahan dikurangi jadi 250ms (lebih responsif)
     longPressTimer.current = setTimeout(() => {
         setIsDragMoveMode(true);
         if (navigator.vibrate) navigator.vibrate(50);
         controls.start({ scale: 1.05, rotate: 2, zIndex: 100 });
     }, 250); 
  };
  ...
  const handlePointerUp = () => {
     ...
     // [FIX #2] Fix Bug "Note Mengambang/Stuck": 
     // Jika user masuk mode drag tapi melepas tanpa menggeser (dragEnd tidak terpanggil),
     // kita harus paksa reset state secara manual di sini.
     if (isDragMoveMode) {
        setIsDragMoveMode(false);
        controls.start({ scale: 1, rotate: 0, x: 0, y: 0, zIndex: 10 });
     }
  };
  ...
  const handleDragEnd = (event: any, info: PanInfo) => {
     if (isDragMoveMode) {
         ...
         // Deteksi elemen di bawah pointer untuk Drop
         const elements = document.elementsFromPoint(info.point.x, info.point.y);
         // Cari elemen yang punya atribut data-folder-id
  ...
  return (
    <motion.div 
       layout 
       transition={springTransition}
       className={`${safeIsPeeking ? 'col-span-2 row-span-2 z-[100]' : 'col-span-1 aspect-square'} relative`}
       onPointerDown={handlePointerDown}
       onPointerUp={handlePointerUp}
       onPointerLeave={handlePointerUp}
       onClick={(e) => { 
           e.stopPropagation(); 
           // Mencegah klik terbuka saat baru selesai drag
           if (!isDragMoveMode) onClick(); 
       }}
       // Mencegah scroll browser native saat drag note
       style={{ touchAction: isDragMoveMode ? 'none' : 'pan-y' }}
    >
        <div className="relative w-full h-full">
            <motion.div
               animate={controls}
               drag={isSelectionMode ? false : (isDragMoveMode ? true : "x")}
               // Jika mode Drag Move: Bebas. Jika Swipe: Horizontal only.
               dragConstraints={isDragMoveMode ? undefined : { left: 0, right: 0 }}
               dragElastic={isDragMoveMode ? 0.2 : { left: 0.5, right: 0.5 }}
               onDragEnd={handleDragEnd}
               ...
               className={`
                  ...
                  ${isDragMoveMode ? 'shadow-2xl cursor-grabbing' : ''}
               `}
            >
               <div className="p-5 h-full flex flex-col relative"> 
                   {safeIsPeeking ? (
                       ...
                       <>
                          <h3 className="text-xl font-bold leading-tight line-clamp-2 relative z-10 pointer-events-none">{note.title || "Untitled"}</h3> 
                          <div className="absolute bottom-4 right-4 opacity-10 pointer-events-none">
                              <Icon size={80} />
                          </div>
                       </>
                   )}
                   {!isSelectionMode && onPeek && !isDragMoveMode && (
                       <button 
